...
import { getPressContent, PRESS_ARCHIVE, PRESS_IMAGES, PRESS_VIDEOS } from "./pressContent";
import "./press.css";
      ...
      <main className="container press-layout">
        <header className="press-intro">
          ...
          <div className="press-actions">
            <ButtonLink href="/#demo">{text.demo}</ButtonLink>
          ...
          <div className="press-grid">
            {PRESS_VIDEOS.map((video) => (
              <figure className="press-card" key={video.id}>
                <video className={video.id.endsWith("4x5") ? "is-portrait" : undefined} controls playsInline preload="none" poster={video.poster} aria-label={`${text.videoTitle} — ${video.label}`}>
                  <source src={video.src} type="video/mp4" />
                  <track kind="captions" src={video.captions} srcLang={video.language} label={video.language === "zh" ? "中文" : "English"} />
                </video>
                <figcaption>
                  <h3>{video.label}</h3>
                  <p className="press-note">{video.resolution} · 33 s · MP4</p>
