...
Because the one-function re-export turns `anydoc` into a local path dependency,
Cargo cannot retain the registry checksum for that crate in `Cargo.lock`.
The build script therefore downloads the source only from `static.crates.io`
and verifies the exact SHA-256 published in the official crates.io index before
extracting it. A version bump must update both the exact dependency pin and the
reviewed checksum.
...
`cargo audit` currently reports two allowed warnings: `ttf-parser` 0.25.1 is
unmaintained (`RUSTSEC-2026-0192`), and the locked `chacha20` 0.10.1 release is
yanked. Both are transitive, neither is reported as a vulnerability, and the
exact fixed-target lock remains reproducible; warnings report without failing
the job.
