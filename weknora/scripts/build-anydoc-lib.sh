...
# The crates.io checksum is deliberately kept next to the exact version pin.
# anydoc becomes a local path dependency after we re-export one function, so
# Cargo no longer records or verifies the registry checksum in Cargo.lock.
# A version bump must therefore update this value from the official crates.io
# index as a separate, reviewable supply-chain decision.
anydoc_checksum_version=0.1.9
anydoc_checksum=93a8b0dbecf79be9329111093b493d256db2a573ed3f1cc827c96bdbbe32a936
if [ "$anydoc_version" != "$anydoc_checksum_version" ]; then
  echo "error: anydoc $anydoc_version has no reviewed crates.io checksum in this script." >&2
  exit 1
fi
...
prepare_patched_anydoc() {
  ...
  if [ -f "$dest/.weknora-patched" ] &&
     [ "$(cat "$dest/.weknora-patched")" = "$anydoc_version" ] &&
     [ -f "$dest/.weknora-source-sha256" ] &&
     [ "$(cat "$dest/.weknora-source-sha256")" = "$anydoc_checksum" ] &&
     [ -f "$dest/Cargo.toml" ]; then
    return
  fi

  local tarball="$crate_dir/.anydoc-$anydoc_version.crate"
  echo "Fetching anydoc $anydoc_version from crates.io to patch document_to_markdown into the public API"
  curl --fail --location --silent --show-error --retry 3 \
    --user-agent 'WeKnora-anydoc-source-build/1.0' \
    "https://static.crates.io/crates/anydoc/anydoc-$anydoc_version.crate" \
    -o "$tarball"
  local actual_checksum
  actual_checksum=$(sha256_file "$tarball")
  if [ "$actual_checksum" != "$anydoc_checksum" ]; then
    echo "error: anydoc $anydoc_version crates.io archive checksum mismatch." >&2
    exit 1
  fi
  ...
  printf '%s\n' "$anydoc_checksum" > "$dest/.weknora-source-sha256"
