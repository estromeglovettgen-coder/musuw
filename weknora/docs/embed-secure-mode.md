...
| 名称 | 格式 | 谁持有 | 用途 |
|------|------|--------|------|
| 发布 Token | `em_…` | 仅你的服务端 | 向 Musuw 换取短时令牌；在管理端「渠道密钥」查看 |
| 会话 Token | `ems_…` | 访客浏览器（iframe 内） | 调聊天、上传等 embed API；约 30 分钟过期，Widget 会自动刷新 |

## 工作流程

```
访客浏览器                 你的后端（shop 的服务器）              Musuw
     │                              │                              │
     │ 1. 加载 Widget               │                              │
     │    data-token-endpoint       │                              │
     │    （不含 em_）               │                              │
...
1. **页面脚本**：`data-token-endpoint="https://你的域名/musuw/embed-token"`（没有 `data-token`）
...
- 用发布 Token 调 Musuw exchange
- 成功时返回 JSON：`{ "token": "<ems_…>", "expiresIn": 1800 }`

**调 exchange 的约定**：

```http
POST https://<musuw-host>/api/v1/embed/<channel_id>/exchange
...
| 取令牌接口 **502** `mint failed` | Musuw 不可达、渠道已停用，或 exchange 响应格式不对 |
| 访客随便就能聊 | 取令牌接口未做登录校验——在 exchange 前加 Session / JWT 检查 |

## 相关

- 可选：embed 独立子域 → [embed-subdomain.md](./embed-subdomain.md)
- Widget SDK 注释：`frontend/public/musuw-widget.js`
