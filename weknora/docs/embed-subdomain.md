...
| **B. Embed 页面源站** | `https://app.example.com` 或 `https://embed.example.com` | 提供 `embed.html`、`musuw-widget.js`；聊天 iframe 加载自这里 |
| **C. Musuw API** | 通常与 B 同域，如 `https://app.example.com/api` | 后端接口 |
...
- 只暴露：`/embed/*` → `embed.html`，`/musuw-widget.js`，`/assets/*`
- `/api/` 反代到 Musuw 后端（与主站相同后端即可）
...
当宿主站 A 与 embed 源站 B **不同域**时，`musuw-widget.js` 会自动给内部 iframe 加上 `sandbox`（也可手动 `data-sandbox="true"`）。
...
- [ ] embed 子域能打开 `/embed/<渠道ID>` 和 `/musuw-widget.js`
- [ ] embed 子域 `/api/` 能连到后端
