...
具名 Sandbox 的后端、凭据、模板、TTL 和私网访问策略均在「设置 → 沙箱后端」按空间保存，不从环境变量继承 provider 身份。为兼容升级前的部署，智能体没有选择具名配置时仍可使用 deployment-wide `WEKNORA_SANDBOX_MODE=docker`（镜像与默认执行超时分别由 `WEKNORA_SANDBOX_DOCKER_IMAGE`、`WEKNORA_SANDBOX_TIMEOUT` 指定）；设为 `disabled` 则禁用。旧 `local` 主机进程模式已因安全原因移除并 fail-closed。
...
**具名空间配置是其 provider 身份的唯一运行时来源。** 端点、凭据和 provider 参数不会从 `.env` 回退；deployment-wide 兼容后端只服务于没有选择具名配置的智能体，不能覆盖具名配置：
