...
空间管理员在「设置 → 沙箱后端」中统一维护 Docker、CubeSandbox 或 E2B 具名配置。远端模板从目标集群实时获取；缺少 WeKnora 标准模板时由系统自动创建。智能体在编辑页选择一份空间配置后以该配置为唯一 provider 来源；没有选择时，升级前的 env-only 部署仍可通过 `WEKNORA_SANDBOX_MODE=docker` 使用 deployment-wide 兼容后端，设为 `disabled` 则不执行技能脚本。旧 `local` 主机进程模式已移除。Docker、CubeSandbox、E2B 均保留会话级沙箱。
