...
func TestOpenRouterAdvertisesEveryCompatiblePlatformModelRole(t *testing.T) {
	info := (&OpenRouterProvider{}).Info()
	for _, modelType := range []types.ModelType{
		...
		assert.Contains(t, info.ModelTypes, modelType)
		assert.Equal(t, OpenRouterBaseURL, info.GetDefaultURL(modelType))
	}
}
