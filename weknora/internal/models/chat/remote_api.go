...
func NewRemoteAPIChat(chatConfig *ChatConfig) (*RemoteAPIChat, error) {
	...
	if providerName == provider.ProviderOpenRouter && chatConfig.OpenRouterMeter != nil {
		meteredHTTPClient = modelopenrouter.WrapHTTPClient(sdkHTTPClient, chatConfig.OpenRouterMeter)
...
func (c *RemoteAPIChat) chatStreamWithRawHTTP(ctx context.Context, endpoint string, customReq any) (<-chan types.StreamResponse, error) {
	...
	logger.Infof(ctx, "[LLM Stream Request] endpoint=%s, model=%s, stream=true", endpoint, c.modelName)
	httpReq, err := http.NewRequestWithContext(ctx, "POST", endpoint, bytes.NewBuffer(jsonData))
	if err != nil {
		return nil, fmt.Errorf("create request: %w", err)
	}
