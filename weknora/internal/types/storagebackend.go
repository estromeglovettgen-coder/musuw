...
func (c StorageBackendConfig) Value() (driver.Value, error) {
	var err error
	if c.AccessKeyID, err = encryptStoredSecretStrict("storage_backends.config.access_key_id", c.AccessKeyID); err != nil {
		return nil, err
	}
	if c.SecretAccessKey, err = encryptStoredSecretStrict("storage_backends.config.secret_access_key", c.SecretAccessKey); err != nil {
		return nil, err
	}
