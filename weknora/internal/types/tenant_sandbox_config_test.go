...
func TestTenantSandboxConfigValueFailsClosedWhenNoKeyIsConfigured(t *testing.T) {
	...
	_, err := cfg.Value()
	require.Error(t, err,
		"sandbox credentials must never be persisted as plaintext without SYSTEM_AES_KEY")
	require.Equal(t, "e2b-secret", cfg.E2B.APIKey,
		"a failed Value call must not mutate the receiver")
	require.Equal(t, "hf-secret", cfg.EnvVars["HF_TOKEN"],
		"a failed Value call must not mutate the receiver map")
}
