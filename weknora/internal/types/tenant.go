...
func (c *TenantSandboxConfig) Value() (driver.Value, error) {
	...
	if c.Cube != nil {
		cube := *c.Cube
		encrypted, err := encryptStoredSecretStrict("tenant.sandbox_config.cube.api_key", cube.APIKey)
		if err != nil {
			return nil, err
		}
		cube.APIKey = encrypted
		cp.Cube = &cube
	}
	if c.E2B != nil {
		e2b := *c.E2B
		encrypted, err := encryptStoredSecretStrict("tenant.sandbox_config.e2b.api_key", e2b.APIKey)
		if err != nil {
			return nil, err
		}
		e2b.APIKey = encrypted
		cp.E2B = &e2b
	}
	...
	if len(c.EnvVars) > 0 {
		...
		for name, value := range c.EnvVars {
			encrypted, err := encryptStoredSecretStrict("tenant.sandbox_config.env_vars."+name, value)
			if err != nil {
				return nil, err
			}
