...
// ErrAuthenticationUnavailable marks a temporary failure of the persistence
// checks that make a native session safe to accept.  Callers must return a
// retryable service-unavailable response rather than treating it as proof that
// the browser credential is invalid.
var ErrAuthenticationUnavailable = errors.New("authentication backend unavailable")

// IsAuthenticationUnavailable reports whether an authentication operation
// failed because its backing state could not be verified.
...
type UserService interface {
	...
	// LoginWithOIDC exchanges the callback code and its browser-bound PKCE
	// verifier, auto-provisions users if needed, and completes login.
	...
	LoginWithOIDC(ctx context.Context, code, redirectURI, codeVerifier string, provisioning types.TenantProvisioningMode) (*types.OIDCCallbackResponse, error)
