...
type OIDCAuthURLResponse struct {
	...
	Nonce string `json:"-"`
	// CodeVerifier is intentionally returned only to the HTTP handler so it can
	// bind this authorization attempt to the initiating browser. It must never
	// be serialized into the authorization response or authorization URL.
