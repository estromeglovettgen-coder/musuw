...
func TestTenantUserEnvVarFailsClosedWhenNoKeyIsConfigured(t *testing.T) {
	t.Setenv("SYSTEM_AES_KEY", "")
	row := &TenantUserEnvVar{Name: "TAVILY_API_KEY", Value: "tvly-super-secret"}

	require.Error(t, row.BeforeSave(nil),
		"a non-empty user env var must never be persisted as plaintext")
	require.Equal(t, "tvly-super-secret", row.Value)
}

func TestSkillEnvVarsValueFailsClosedWhenNoKeyIsConfigured(t *testing.T) {
	t.Setenv("SYSTEM_AES_KEY", "")
	vars := SkillEnvVars{{Name: "TAVILY_API_KEY", Value: "tvly-super-secret"}}

	_, err := vars.Value()
	require.Error(t, err,
		"a non-empty skill env var must never be persisted as plaintext")
}
