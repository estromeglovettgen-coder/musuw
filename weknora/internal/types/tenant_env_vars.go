...
// encryptEnvValue enforces encrypted-at-rest persistence for non-empty values.
// Missing or invalid SYSTEM_AES_KEY is a bounded write error rather than a
// plaintext fallback. Existing ciphertext remains idempotent so a row can be
// re-saved while an operator is repairing key configuration; reads are still
// deliberately lenient in Scan/AfterFind.
func encryptEnvValue(value string) (string, error) {
	return encryptStoredSecretStrict("tenant environment variable", value)
}
