...
// ApplyPlatformKnowledgeBaseDefaults turns a document KB create request into
// the platform's zero-configuration product profile. It is intentionally used
// only by the create service; reads and edits preserve existing KB choices.
func (kb *KnowledgeBase) ApplyPlatformKnowledgeBaseDefaults() {
	if kb == nil {
		return
	}
	if kb.Type == "" {
	...
	// existing graph extractor.
	kb.ExtractConfig = &ExtractConfig{
		Enabled: true,
		Text:    `"Romeo and Juliet" is a tragedy written by William Shakespeare and set in Verona.`,
		Tags:    []string{"Author", "Alias", "Setting"},
		Nodes: []*GraphNode{
			{Name: "Romeo and Juliet", Attributes: []string{"A tragedy"}},
			{Name: "William Shakespeare", Attributes: []string{"English playwright"}},
			{Name: "Verona", Attributes: []string{"City in Italy"}},
		},
