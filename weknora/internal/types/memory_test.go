...
func TestRedactSensitiveRemovesCredentials(t *testing.T) {
	cases := []struct {
		...
		// Split credential-shaped fixtures in source so the repository's
		// high-confidence secret scanner can still reject an accidentally
		// committed real token. Go concatenates these to the exact runtime
		// strings exercised by the redactor.
		{"github token", "用 ghp_" + "ABCDEFGHIJKLMNOPQRSTUVWXYZ012345 拉代码"},
		{"aws key", "AKIA" + "IOSFODNN7EXAMPLE 是我们的 access key"},
