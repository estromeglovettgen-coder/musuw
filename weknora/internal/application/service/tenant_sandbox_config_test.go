...
func testGlobalSandboxConfig() *sandbox.Config {
	...
	// Keep endpoint fixtures hermetic. Some CI DNS shims resolve public names
	// into RFC 2544's synthetic 198.18.0.0/15 range, which the outbound guard
...
func e2bCfg(key, url, domain, template string, ttl int) *types.TenantSandboxConfig {
	// The endpoint values in the table below are descriptive aliases from the
	// upstream tests. Use TEST-NET literals so validation does not depend on
	// external DNS (or a CI DNS shim mapping those names to 198.18/15).
	switch url {
	case "https://api.e2b.app":
		url = "https://203.0.113.10"
	case "https://self.hosted":
