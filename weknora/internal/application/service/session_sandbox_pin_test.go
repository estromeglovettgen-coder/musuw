...
func TestResolveSandboxForExecutionUsesDeploymentDefaultAndPinsSentinel(t *testing.T) {
	pinner := NewSessionSandboxPinner(newPinTestDB(t))
	want := &pinTestManager{typ: sandbox.SandboxTypeDocker}

	got, configID, err := resolveSandboxForExecution(
		context.Background(), nil, want, pinner,
		7, "s-1", "", nil,
	)

	require.NoError(t, err)
	require.Same(t, want, got)
	require.Equal(t, types.SandboxConfigIDGlobalDefault, configID)
	pinned, err := pinner.Read(context.Background(), "s-1")
	require.NoError(t, err)
	require.Equal(t, types.SandboxConfigIDGlobalDefault, pinned,
		"env-only default backends need a stable pin for artifact collection and teardown")
}
