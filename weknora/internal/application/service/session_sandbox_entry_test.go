...
func (m *artifactFallbackManager) ReadSessionFileBounded(
	ctx context.Context, sessionID, path string, maxBytes int64,
) ([]byte, error) {
	bounded, ok := m.source.(sandbox.SessionBoundedFileReader)
	if !ok {
		return nil, stderrors.New("artifact source does not support bounded reads")
	}
	return bounded.ReadSessionFileBounded(ctx, sessionID, path, maxBytes)
}
