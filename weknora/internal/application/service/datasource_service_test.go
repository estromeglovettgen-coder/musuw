...
// TestProcessSync_SyncDeletionsCountsOnly verifies Musuw's source-deletion
// policy: tombstones are reported in the sync result, while knowledge rows
// remain untouched for explicit user cleanup in the KB UI.
...
func TestProcessSync_SyncDeletionsDeletesMatchingKnowledge(t *testing.T) {
	...
	assert.Empty(t, h.knowledgeRepo.externalID)
}
...
// TestProcessSync_SyncDeletionsIgnoresLookup verifies the count-only path does
// not inspect repository state, even when no matching row exists.
...
// TestProcessSync_SyncDeletionsLookupFailureCountsOnly verifies repository
// lookup failures cannot turn a source tombstone into a sync failure because
// count-only semantics do not perform lookups.
func TestProcessSync_SyncDeletionsLookupFailureCountsFailed(t *testing.T) {
	...
	assert.Empty(t, h.knowledgeSvc.deleted)
	assert.Equal(t, 1, updated.ItemsDeleted)
	assert.Equal(t, 0, updated.ItemsFailed)
	result, err := updated.ParseResult()
	require.NoError(t, err)
	assert.Empty(t, result.Errors)
	assert.Equal(t, 0, deletionFailedCount(t, updated))
}

// TestProcessSync_SyncDeletionsDeleteFailureCountsOnly verifies a configured
// delete failure is irrelevant because source tombstones never call delete.
...
func TestProcessSync_SyncDeletionsCountOnlyWhenMixedResults(t *testing.T) {
