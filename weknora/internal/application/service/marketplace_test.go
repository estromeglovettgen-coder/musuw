...
func (g *marketplaceGatewayStub) Config() types.MarketplaceCheckoutConfig {
	return types.MarketplaceCheckoutConfig{Environment: "sandbox", ClientToken: "test_unit", Configured: true, PortalConfigured: true}
}
func (g *marketplaceGatewayStub) ValidateCatalog(context.Context, *types.MarketplaceProduct) error {
	return nil
}
func (g *marketplaceGatewayStub) CreateCheckout(context.Context, *types.MarketplaceSubscription) (string, error) {
	g.createCalls++
	if g.createErr != nil {
		return "", g.createErr
	}
...
func (g *marketplaceGatewayStub) RecoverCheckout(context.Context, *types.MarketplaceSubscription) (*types.MarketplaceCheckoutRecovery, error) {
...
func TestMarketplaceCheckoutRecoversUncertainWriteWithoutChargingTwice(t *testing.T) {
	...
	gateway := &marketplaceGatewayStub{recovery: &types.MarketplaceCheckoutRecovery{TransactionID: "txn_recovered", Status: "ready"}}
	svc := NewMarketplaceService(repo, &marketplaceEntitlementStub{plan: types.ConsumerPlanFree}, repository.NewKnowledgeBaseRepository(db), repository.NewCustomAgentRepository(db), gateway)
	require.NoError(t, db.Create(&types.MarketplaceProduct{ID: "product", CreatorTenantID: 2, CreatorUserID: "creator", Title: "Product", AgentID: "agent", KnowledgeBaseIDs: types.StringArray{}, KnowledgeBaseNames: types.StringArray{}, PlatformKnowledgeBaseIDs: types.StringArray{}, SampleQuestions: types.StringArray{}, MonthlyAmount: 500, YearlyAmount: 5000, MonthlyPriceID: "pri_monthly", YearlyPriceID: "pri_yearly", Currency: "USD", Status: "published"}).Error)
	ago := time.Now().Add(-5 * time.Minute)
	pending := &types.MarketplaceSubscription{ID: "local", TenantID: 7, UserID: "buyer", ProductID: "product", ProductTitle: "Product", OperationKey: "operation-recovery-0001", BillingPeriod: "monthly", PriceID: "pri_monthly", Currency: "USD", Amount: 500, Status: "uncertain", CreatedAt: ago, UpdatedAt: ago}
...
func TestMarketplaceUnknownCheckoutOutcomeKeepsFenceAndPortalChecksBuyer(t *testing.T) {
	...
	svc := NewMarketplaceService(repo, &marketplaceEntitlementStub{plan: types.ConsumerPlanFree}, repository.NewKnowledgeBaseRepository(db), repository.NewCustomAgentRepository(db), gateway)
	require.NoError(t, db.Create(&types.MarketplaceProduct{ID: "product", CreatorTenantID: 2, CreatorUserID: "creator", Title: "Product", AgentID: "agent", KnowledgeBaseIDs: types.StringArray{}, KnowledgeBaseNames: types.StringArray{}, PlatformKnowledgeBaseIDs: types.StringArray{}, SampleQuestions: types.StringArray{}, MonthlyAmount: 100, YearlyAmount: 1000, MonthlyPriceID: "pri_monthly", YearlyPriceID: "pri_yearly", Currency: "USD", Status: "published"}).Error)
	...
	require.NoError(t, db.Model(sub).Updates(map[string]any{"paddle_customer_id": "ctm_owned", "paddle_subscription_id": "sub_owned", "status": "past_due"}).Error)
...
func TestMarketplaceAccountDeletionRetriesUnknownCheckoutAndSkipsBoundSubscriptions(t *testing.T) {
	...
	rows := []*types.MarketplaceSubscription{
		{ID: "never-sent", TenantID: 7, UserID: "buyer", ProductID: "one", OperationKey: "one", Status: "pending", CreatedAt: ago, UpdatedAt: ago},
		{ID: "unknown", TenantID: 7, UserID: "buyer", ProductID: "two", OperationKey: "two", Status: "uncertain", CreatedAt: ago, UpdatedAt: ago},
		{ID: "bound", TenantID: 7, UserID: "buyer", ProductID: "three", OperationKey: "three", Status: "active", PaddleCustomerID: "ctm_bound", PaddleSubscriptionID: "sub_bound", CreatedAt: ago, UpdatedAt: ago},
	}
...
func TestMarketplaceApprovalUsesPlatformSnapshotAndHidesSubmissionSecrets(t *testing.T) {
	db := marketplaceServiceTestDB(t)
	repo := repository.NewMarketplaceRepository(db)
	svc := NewMarketplaceService(repo, &marketplaceEntitlementStub{plan: types.ConsumerPlanMax}, repository.NewKnowledgeBaseRepository(db), repository.NewCustomAgentRepository(db), &marketplaceGatewayStub{})
	require.NoError(t, db.Create(&types.CustomAgent{ID: "source-agent", TenantID: 7, Name: "Source", CreatedBy: "creator"}).Error)
	require.NoError(t, db.Create(&types.KnowledgeBase{ID: "source-kb", TenantID: 7, Name: "Source KB"}).Error)
	require.NoError(t, db.Create(&types.CustomAgent{ID: "platform-agent", TenantID: 99, Name: "Reviewed agent", Config: types.CustomAgentConfig{SystemPrompt: "Reviewed private prompt", ModelID: "private-model", RerankModelID: "private-rerank", MCPSelectionMode: "all", SkillsSelectionMode: "all", SandboxConfigID: "private-sandbox"}}).Error)
	require.NoError(t, db.Create(&types.KnowledgeBase{ID: "platform-kb", TenantID: 99, Name: "Reviewed knowledge", EmbeddingModelID: types.PlatformKnowledgeBaseEmbeddingModelID}).Error)
	creator := marketplaceIdentity(7, "creator", false)
	p, err := svc.SaveProduct(creator, "", types.MarketplaceProductInput{Title: "Reviewed service", Description: "A reviewed knowledge service", AgentID: "source-agent", KnowledgeBaseIDs: []string{"source-kb"}, MonthlyAmount: 500, Contact: "private@example.com", Authorization: "Private contract text", AuthorizationConfirmed: true}, false)
	require.NoError(t, err)
	_, err = svc.SubmitProduct(creator, p.ID)
	require.NoError(t, err)
	review := types.MarketplaceReviewInput{Action: "approve", PlatformAgentID: "platform-agent", PlatformKnowledgeBaseIDs: []string{"platform-kb"}, PaddleProductID: "pro_reviewed", MonthlyPriceID: "pri_monthly", YearlyPriceID: "pri_yearly", ReviewNote: "private internal review"}
	...
	require.NoError(t, err)
	for _, secret := range []string{"private@example.com", "Private contract text", "private internal review", "Reviewed private prompt", "creator_tenant_id", "creator_name", "source-agent", "source-kb"} {
		require.NotContains(t, string(raw), secret)
	}
	end := time.Now().Add(time.Hour)
	require.NoError(t, db.Create(&types.MarketplaceSubscription{ID: "paid", TenantID: 8, UserID: "buyer", ProductID: p.ID, ProductTitle: p.Title, OperationKey: "purchase", BillingPeriod: "monthly", PriceID: "pri_monthly", Currency: "USD", Amount: 500, Status: "active", PaidThrough: &end}).Error)
	...
	require.NoError(t, db.Model(&types.CustomAgent{}).Where("id = ? AND tenant_id = ?", "platform-agent", 99).Update("config", types.CustomAgentConfig{SystemPrompt: "Unreviewed later edit"}).Error)
	access, err = svc.AuthorizeAccess(buyer, 8, p.ID, time.Now())
	require.NoError(t, err)
...
func marketplaceServiceTestDB(t *testing.T) *gorm.DB {
	...
	require.NoError(t, db.AutoMigrate(&types.MarketplaceProduct{}, &types.MarketplaceSubscription{}, &types.MarketplaceTransaction{}, &types.MarketplaceProcessedEvent{}, &types.KnowledgeBase{}, &types.CustomAgent{}, &types.User{}))
...
func TestMarketplaceOnlyMaxCanCreateOwnedSubmissionAndYearCostsTenMonths(t *testing.T) {
	...
	svc := NewMarketplaceService(repository.NewMarketplaceRepository(db), ent, repository.NewKnowledgeBaseRepository(db), repository.NewCustomAgentRepository(db), nil)
	ctx := marketplaceIdentity(7, "creator", false)
	require.NoError(t, db.Create(&types.CustomAgent{ID: "agent", TenantID: 7, Name: "Creator agent", CreatedBy: "creator"}).Error)
	require.NoError(t, db.Create(&types.KnowledgeBase{ID: "kb", TenantID: 7, Name: "Creator knowledge", CreatorID: "creator"}).Error)
	input := types.MarketplaceProductInput{Title: "Reviewed knowledge", Description: "A useful curated source", Category: "learning", AgentID: "agent", KnowledgeBaseIDs: []string{"kb"}, MonthlyAmount: 500, Contact: "creator@example.com", Authorization: "I own and authorize this content.", AuthorizationConfirmed: true}
