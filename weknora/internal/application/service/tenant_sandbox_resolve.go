...
func resolveTenantSandboxForConfig(
	...
	// ② No named workspace config uses the deployment-wide manager. This is the
	// compatibility path for env-only deployments (notably the historical
	// WEKNORA_SANDBOX_MODE=docker setup). A nil manager still fails closed as
	// disabled; named configs never use this fallback.
