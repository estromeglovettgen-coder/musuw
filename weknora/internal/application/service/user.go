...
func (s *userService) GetOIDCAuthorizationURL(ctx context.Context, redirectURI string) (*types.OIDCAuthURLResponse, error) {
	...
	// Supabase OAuth 2.1 requires authorization-code PKCE with S256. A 32-byte
	// base64url verifier is 43 characters, the RFC 7636 minimum.
...
func (s *userService) ValidateToken(ctx context.Context, tokenString string) (*types.User, uint64, error) {
	...
	if err != nil {
		...
		return nil, 0, authenticationUnavailable("look up access token", err)
	}
	...
	if err != nil {
		...
		return nil, 0, authenticationUnavailable("look up token user", err)
	}
	if user == nil {
		return nil, 0, errors.New("token user no longer exists")
	}
...
func authenticationUnavailable(operation string, cause error) error {
	return fmt.Errorf("%w: %s: %v", interfaces.ErrAuthenticationUnavailable, operation, cause)
}
...
func (s *userService) RefreshToken(
	...
	if err != nil {
		...
		return "", "", authenticationUnavailable("look up refresh token", err)
	}
	if tokenRecord == nil || tokenRecord.IsRevoked {
		return "", "", errors.New("refresh token is revoked")
	}
	...
	if err != nil {
		...
		return "", "", authenticationUnavailable("look up refresh token user", err)
	}
	if user == nil {
		return "", "", errors.New("refresh token user no longer exists")
	}
...
func (s *userService) exchangeOIDCCode(ctx context.Context, cfg *config.OIDCAuthConfig, code, redirectURI, codeVerifier string) (*oidcTokenResponse, error) {
	if strings.TrimSpace(codeVerifier) == "" {
		return nil, errors.New("code_verifier is required")
	}
	if err := validateOIDCEndpoint("token", cfg.TokenEndpoint, true); err != nil {
	...
	// The configured Supabase confidential client uses client_secret_basic.
	// Keep credentials out of the form body and never include them in errors.
...
func (s *userService) provisionOIDCUser(
	...
	identityProvider = strings.ToLower(strings.TrimSpace(identityProvider))
	if identityProvider == "" || strings.TrimSpace(info.Subject) == "" {
		return nil, ErrOIDCIdentityBindingInvalid
	}
	...
	// Register has already persisted the new row. Set the external identity
	// and OIDC-only preference before the single follow-up update so a fresh
	// account is never visible in a partially bound state and the provisioning
	// path performs one write instead of updating identity and preferences
	// independently.
	user.IdentityProvider = identityProvider
	user.IdentitySubject = strings.TrimSpace(info.Subject)
	oidcOnly := true
