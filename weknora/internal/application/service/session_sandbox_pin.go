...
func resolveSandboxForExecution(
	...
	if configID == "" {
		// The deployment-wide manager is a real session-scoped backend when a
		// legacy env-only Docker deployment is enabled. Give it the same stable
		// identity as other backends so the session pin, artifact collector and
		// teardown all resolve that manager instead of treating an empty pin as
		// "no sandbox".
