...
func (s *knowledgeService) convertVideo(
	...
	if model.GetModelName() != vlm.OpenRouterGeminiVideoModel {
		err = fmt.Errorf("video understanding requires %s", vlm.OpenRouterGeminiVideoModel)
		s.failStage(ctx, knowledge.ID, types.StageDocReader,
			werrors.ErrCodeDocReaderParseFailed, "invalid video model", err)
