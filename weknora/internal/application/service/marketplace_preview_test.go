...
func TestMarketplacePreviewShowsOnlyPublishedBoundDirectoryAndCuratedAnswers(t *testing.T) {
	...
	for _, kb := range []types.KnowledgeBase{{ID: "published-kb", TenantID: 8, Name: "泰勒"}, {ID: "other-kb", TenantID: 8, Name: "Private"}} {
		require.NoError(t, db.Create(&kb).Error)
	}
	require.NoError(t, db.Create(&types.WikiFolder{ID: "root", TenantID: 8, KnowledgeBaseID: "published-kb", Name: "真实分类"}).Error)
	require.NoError(t, db.Create(&types.WikiFolder{ID: "nested", TenantID: 8, KnowledgeBaseID: "published-kb", ParentID: "root", Name: "沟通"}).Error)
	for _, page := range []types.WikiPage{
		{ID: "page", TenantID: 8, KnowledgeBaseID: "published-kb", FolderID: "nested", Slug: "concept/communication", Title: "真实文章", PageType: "concept", Status: "published", Content: "PRIVATE_BODY", Summary: "PRIVATE_SUMMARY"},
		{ID: "draft", TenantID: 8, KnowledgeBaseID: "published-kb", Slug: "draft", Title: "HIDDEN_DRAFT", PageType: "concept", Status: "draft"},
		{ID: "foreign", TenantID: 9, KnowledgeBaseID: "published-kb", Slug: "foreign", Title: "HIDDEN_TENANT", PageType: "concept", Status: "published"},
		{ID: "unbound", TenantID: 8, KnowledgeBaseID: "other-kb", Slug: "other", Title: "HIDDEN_KB", PageType: "concept", Status: "published"},
		{ID: "index", TenantID: 8, KnowledgeBaseID: "published-kb", Slug: "index", Title: "HIDDEN_INDEX", PageType: "index", Status: "published"},
	} {
		require.NoError(t, db.Create(&page).Error)
	}
	p := &types.MarketplaceProduct{ID: "product", CreatorTenantID: 2, CreatorUserID: "creator", PublishedTenantID: 8, PlatformKnowledgeBaseIDs: types.StringArray{"published-kb"}, Title: "Taylor", Status: "published", SampleConversations: []types.MarketplaceExample{{Question: "真实问题", Answer: "精选最终回答"}}}
	...
	require.Equal(t, []types.MarketplaceDirectoryEntry{{ID: "page", Title: "真实文章", Path: []string{"泰勒", "真实分类", "沟通"}, PageType: "concept"}}, preview.Directory)
	require.Equal(t, p.SampleConversations, preview.Examples)
	raw, err := json.Marshal(preview)
	require.NoError(t, err)
	for _, forbidden := range []string{"PRIVATE", "HIDDEN", "tenant_id", "summary", "source_refs", "session_id", "agent_steps"} {
...
func TestMarketplacePreviewExamplesAreAdminCuratedAndNotInCatalog(t *testing.T) {
	...
	svc := NewMarketplaceService(repo, &marketplaceEntitlementStub{plan: types.ConsumerPlanMax}, repository.NewKnowledgeBaseRepository(db), repository.NewCustomAgentRepository(db), nil)
	...
	input := types.MarketplaceProductInput{Title: "Product", Description: "Intro", AgentID: "agent", KnowledgeBaseIDs: []string{"kb"}, MonthlyAmount: 100, SampleConversations: &examples}
	...
	listed, _, err := svc.ListProducts(marketplaceIdentity(7, "buyer", false), interfaces.MarketplaceCatalogQuery{}, false, false)
	require.NoError(t, err)
