...
func (f *fakeSandboxSource) ReadSessionFileBounded(
	ctx context.Context, sessionID, path string, maxBytes int64,
) ([]byte, error) {
	f.boundedReadLimits = append(f.boundedReadLimits, maxBytes)
	data, err := f.ReadSessionFile(ctx, sessionID, path)
	if err != nil {
		return nil, err
	}
	// Model the provider-side cap+1 contract: the collector receives no more
	// than one byte beyond its configured limit, even when the backing fixture
	// advertises a stale smaller size.
	limit := maxBytes + 1
...
func newTestCollector(src SandboxArtifactSource, store *fakeStore, fs *fakeFileService, max int64) *ArtifactCollector {
...
func TestArtifactCollector_SkipsOversizeAfterRead(t *testing.T) {
	// Envd can report a stale size while the file grows; the provider-side
	// bounded read must cap the materialised payload at max+1 before the
	// collector enforces the cap against the actual bytes.
	ctx := context.Background()
	src := &fakeSandboxSource{
		entries: map[string][]sandbox.RemoteDirEntry{
			"sess-1": {
	...
	if len(src.boundedReadLimits) != 1 || src.boundedReadLimits[0] != 100 {
		t.Fatalf("bounded read limits = %v, want [100]", src.boundedReadLimits)
	}
	if len(src.readCalls) != 1 {
		t.Fatalf("ReadSessionFile calls = %d, want 1", len(src.readCalls))
	}
	boundedData, err := src.ReadSessionFileBounded(ctx, "sess-1", "/workspace/output/lying.bin", 100)
	if err != nil {
		t.Fatalf("ReadSessionFileBounded() error = %v", err)
	}
	if len(boundedData) != 101 {
		t.Fatalf("bounded payload length = %d, want cap+1=101", len(boundedData))
	}
}

func TestArtifactCollectorSkipsSourceWithoutBoundedRead(t *testing.T) {
	ctx := context.Background()
	src := &unboundedArtifactSource{
		entries: []sandbox.RemoteDirEntry{{
			Name: "report.txt", Path: "/workspace/output/report.txt",
			Type: sandbox.RemoteEntryFile, Size: 1,
		}},
		content: []byte("ok"),
	}
	fs := &fakeFileService{}
	...
	if len(got) != 0 || len(fs.saved) != 0 {
		t.Fatalf("unbounded source must be skipped: artifacts=%v saved=%v", got, fs.saved)
	}
}

type unboundedArtifactSource struct {
	entries []sandbox.RemoteDirEntry
	content []byte
}

func (s *unboundedArtifactSource) ListSessionFiles(context.Context, string, string) ([]sandbox.RemoteDirEntry, error) {
	return s.entries, nil
}

func (s *unboundedArtifactSource) ReadSessionFile(context.Context, string, string) ([]byte, error) {
