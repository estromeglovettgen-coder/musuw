...
// defaultMaxArtifactFileBytes caps a single artifact at 50 MiB. The sandbox
// source enforces this limit provider-side (reading at most cap+1 bytes), so a
// stale directory listing cannot make the process materialise an unbounded
// artifact before this collector rejects it.
...
func (c *ArtifactCollector) maybePersist(
	...
	boundedSource, ok := source.(sandbox.SessionBoundedFileReader)
	if !ok {
		logger.Warnf(ctx,
			"[ArtifactCollector] skip artifact: sandbox source lacks bounded reads: session=%s path=%s",
			sessionID, entry.Path,
		)
		return types.MessageArtifact{}, false
	}
	data, err := boundedSource.ReadSessionFileBounded(ctx, sessionID, entry.Path, c.config.MaxFileBytes)
	if err != nil {
