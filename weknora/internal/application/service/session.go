...
func (s *sessionService) holdSandboxTurn(
	...
	// A blank ID (or the explicit global sentinel used by legacy env-only
	// Docker deployments) means the deployment-wide manager. Only this path
	// may touch s.sandboxMgr directly. Named configs must resolve first: using
	// the fallback before resolving would open a lease on the wrong backend and
	// leave the selected Cube/E2B/Docker session unprotected during a turn.
	...
	if configID == "" || configID == types.SandboxConfigIDGlobalDefault {
		...
		resolved, err := resolveTenantSandboxForConfig(
