...
func TestOIDCTokenExchangeBlocksRedirectToInternalURL(t *testing.T) {
	...
	_, err := svc.exchangeOIDCCode(context.Background(), cfg, "code", "https://app.example/callback", "test-pkce-verifier-0123456789abcdefghijklmnopqrstuvwxyz")
...
func TestOIDCErrorsDoNotEchoSecrets(t *testing.T) {
	withOIDCSSRFWhitelist(t, "127.0.0.1")
	const codeVerifier = "test-pkce-verifier-0123456789abcdefghijklmnopqrstuvwxyz"

	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		w.WriteHeader(http.StatusBadRequest)
		_, _ = w.Write([]byte("client-secret bearer-token " + codeVerifier))
	}))
	defer server.Close()

	svc := &userService{}
	cfg := &config.OIDCAuthConfig{TokenEndpoint: server.URL, ClientID: "client-id", ClientSecret: "client-secret"}
	_, err := svc.exchangeOIDCCode(context.Background(), cfg, "code", "https://app.example/callback", codeVerifier)
	if err == nil {
	...
	if strings.Contains(err.Error(), codeVerifier) {
		t.Fatalf("exchangeOIDCCode error leaked PKCE verifier: %v", err)
	}
