	...
	return func(c *gin.Context) {
		...
		if token, ok := bearerToken(c); ok {
			...
			if interfaces.IsAuthenticationUnavailable(err) {
				logger.Warnf(c.Request.Context(), "[auth] native token verification temporarily unavailable: %v", err)
				_ = c.Error(apperrors.NewServiceUnavailableError("authentication service temporarily unavailable"))
				c.Abort()
				return
			}
...
func authenticateJWTUser(
	...
	if tenant == nil {
		...
		if err != nil {
			logger.Warnf(ctx, "[auth] tenant lookup temporarily unavailable: tenant=%d user=%s err=%v", targetTenantID, user.ID, err)
			_ = c.Error(apperrors.NewServiceUnavailableError("authentication service temporarily unavailable"))
			c.Abort()
			return false
		}
	...
	role, ok, membershipErr := resolveTenantRole(ctx, memberService, user, targetTenantID, crossTenantSwitch, cfg)
	if membershipErr != nil {
		logger.Warnf(ctx, "[auth] tenant membership lookup temporarily unavailable: user=%s tenant=%d err=%v", user.ID, targetTenantID, membershipErr)
		_ = c.Error(apperrors.NewServiceUnavailableError("authentication service temporarily unavailable"))
		c.Abort()
		return false
	}
...
func resolveTargetTenant(
	...
	if tenantHeader := c.GetHeader("X-Tenant-ID"); tenantHeader != "" {
		...
		// 有 active membership 行——三选一。成员状态暂时不可读不是拒绝令牌。
		accessible, accessErr := tenantAccessibility(ctx, user, parsedTenantID, memberService, cfg)
		if accessErr != nil {
			logger.Warnf(ctx, "Target tenant membership lookup temporarily unavailable: tenantID=%d user=%s err=%v", parsedTenantID, user.ID, accessErr)
			_ = c.Error(apperrors.NewServiceUnavailableError("authentication service temporarily unavailable"))
			c.Abort()
			return 0, nil, false, false
		}
		...
		if err != nil {
			logger.Warnf(ctx, "Target tenant lookup temporarily unavailable: tenantID=%d err=%v", parsedTenantID, err)
			_ = c.Error(apperrors.NewServiceUnavailableError("authentication service temporarily unavailable"))
			c.Abort()
			return 0, nil, false, false
		}
...
// applies; false only when we want callers to abort with 403. The final error
// reports a membership persistence failure, which callers must render as 503
// rather than treating it as a rejected credential.
	...
	if isHomeTenant {
		...
		if !hasAny {
			if _, e := memberService.AddMember(
				...
				return types.TenantRoleOwner, true, nil
			} else {
