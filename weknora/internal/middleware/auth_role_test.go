...
func TestResolveTenantRole_ActiveMembershipWins(t *testing.T) {
	...
	got, ok, err := resolveTenantRole(context.Background(), svc,
		&types.User{ID: "u1", TenantID: 10}, 10, false, cfgWithRBAC(true))
	if err != nil || !ok || got != types.TenantRoleContributor {
...
func TestResolveTenantRole_CrossTenantSuperuserGetsAdmin_NoAutoPromote(t *testing.T) {
	...
	got, ok, err := resolveTenantRole(context.Background(), svc, user, 99, true, cfgWithRBAC(true))
...
func TestResolveTenantRole_AutoPromoteRequiresHomeTenant(t *testing.T) {
	...
	got, ok, err := resolveTenantRole(context.Background(), svc, user, 42, true, cfgWithRBAC(true))
	if err != nil || !ok || got != types.TenantRoleAdmin {
...
func TestResolveTenantRole_AutoPromoteHomeTenant(t *testing.T) {
	...
	got, ok, err := resolveTenantRole(context.Background(), svc, user, 7, false, cfgWithRBAC(true))
	if err != nil || !ok || got != types.TenantRoleOwner {
...
func TestResolveTenantRole_FailOpenAdminWhenRBACDisabled(t *testing.T) {
	...
	got, ok, err := resolveTenantRole(context.Background(), svc, user, 8, false, cfgWithRBAC(false))
	if err != nil || !ok || got != types.TenantRoleAdmin {
...
func TestResolveTenantRole_FailClosedWhenRBACEnabled(t *testing.T) {
	...
	user := &types.User{ID: "u1", TenantID: 7}
	if _, ok, err := resolveTenantRole(context.Background(), svc, user, 8, false, cfgWithRBAC(true)); err != nil || ok {
		t.Fatalf("EnableRBAC=true + no membership should be rejected")
	}
}

func TestResolveTenantRole_LookupErrorIsUnavailable(t *testing.T) {
	// Temporary persistence failures are not proof of a missing membership.
	svc := newFakeMemberService()
	...
	got, ok, err := resolveTenantRole(context.Background(), svc, user, 8, false, cfgWithRBAC(false))
	if err == nil || ok || got != "" {
		t.Fatalf("transient lookup error must be unavailable, got (%v, %v, %v)", got, ok, err)
	}
}

func TestResolveTenantRole_HomeMembershipPersistenceErrorsAreUnavailable(t *testing.T) {
	for _, scenario := range []struct {
		name  string
		setup func(*fakeMemberService)
	}{
		{
			name: "member count",
			setup: func(svc *fakeMemberService) {
				svc.failHasAny = errors.New("transient member count failure")
			},
		},
		{
			name: "owner repair",
			setup: func(svc *fakeMemberService) {
				svc.failAdd = errors.New("transient owner repair failure")
			},
		},
	} {
		t.Run(scenario.name, func(t *testing.T) {
			svc := newFakeMemberService()
			scenario.setup(svc)
			got, ok, err := resolveTenantRole(
				context.Background(), svc, &types.User{ID: "u1", TenantID: 7}, 7, false, cfgWithRBAC(true),
			)
			if err == nil || ok || got != "" {
				t.Fatalf("membership persistence failure must be unavailable, got (%v, %v, %v)", got, ok, err)
			}
		})
	}
}
...
func TestResolveTenantRole_DemotedUserCannotReclaimViaOrphan(t *testing.T) {
	...
	got, ok, err := resolveTenantRole(context.Background(), svc, user, 5, false, cfgWithRBAC(true))
	if err != nil || !ok || got != types.TenantRoleOwner {
