...
//
// Lookup errors are treated as "not a member" by this compatibility helper.
// Authentication itself uses tenantAccessibility so it can return a retryable
// 503 rather than misreporting a persistence outage as a denied session.
func IsTenantAccessible(
	...
	allowed, _ := tenantAccessibility(ctx, user, targetTenantID, memberService, cfg)
	return allowed
}

func tenantAccessibility(
	ctx context.Context,
	user *types.User,
	targetTenantID uint64,
	memberService interfaces.TenantMemberService,
	...
	return m.Status == types.TenantMemberStatusActive, nil
}
