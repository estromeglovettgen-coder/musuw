...
func TestParseIMAConfig(t *testing.T) {
	// Keep config parsing deterministic behind VPN/proxy DNS implementations
	// that synthesize RFC 2544 answers for public names. The dedicated
	// loopback case below still exercises the production SSRF rejection.
	utils.SetSSRFWhitelistFromRaw("ima.qq.com")
