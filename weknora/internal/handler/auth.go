...
func (h *AuthHandler) GetOIDCAuthorizationURL(c *gin.Context) {
	...
	// Bind the signed-state nonce and PKCE verifier to the initiating browser.
...
func decodeOIDCState(raw string, req *http.Request) (*oidcStatePayload, error) {
	...
	cookie, err := req.Cookie(oidcBindingCookieName)
	if err != nil || cookie == nil || strings.TrimSpace(cookie.Value) == "" {
		return nil, errors.NewValidationError("oidc browser binding cookie missing")
	}
	binding, err := decodeOIDCBrowserBinding(cookie.Value)
	if err != nil {
		return nil, err
	}
...
type oidcBrowserBinding struct {
	Nonce        string `json:"nonce"`
	CodeVerifier string `json:"code_verifier"`
}

func encodeOIDCBrowserBinding(nonce, codeVerifier string) (string, error) {
	if strings.TrimSpace(nonce) == "" || strings.TrimSpace(codeVerifier) == "" {
		return "", errors.NewValidationError("oidc browser binding is incomplete")
	}
	raw, err := json.Marshal(oidcBrowserBinding{Nonce: nonce, CodeVerifier: codeVerifier})
	if err != nil {
		return "", err
	}
...
func decodeOIDCBrowserBinding(raw string) (*oidcBrowserBinding, error) {
	decoded, err := base64.RawURLEncoding.DecodeString(strings.TrimSpace(raw))
	if err != nil {
		return nil, errors.NewValidationError("oidc browser binding is invalid")
	}
	var binding oidcBrowserBinding
	if err := json.Unmarshal(decoded, &binding); err != nil {
		return nil, errors.NewValidationError("oidc browser binding is invalid")
	}
	if strings.TrimSpace(binding.Nonce) == "" || strings.TrimSpace(binding.CodeVerifier) == "" {
		return nil, errors.NewValidationError("oidc browser binding is incomplete")
	}
	return &binding, nil
}

func setOIDCBrowserBinding(c *gin.Context, value string, maxAge int) {
	secure := c.Request.TLS != nil || strings.EqualFold(c.GetHeader("X-Forwarded-Proto"), "https")
	c.SetSameSite(http.SameSiteLaxMode)
	c.SetCookie(oidcBindingCookieName, value, maxAge, "/", "", secure, true)
}
...
func (h *AuthHandler) RefreshToken(c *gin.Context) {
	...
	if err != nil {
		...
		if interfaces.IsAuthenticationUnavailable(err) {
			c.Error(errors.NewServiceUnavailableError("authentication service temporarily unavailable"))
			return
		}
