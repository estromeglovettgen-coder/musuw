...
func markKnowledgeCreditExhausted(
	...
	// Do not clear StorageKey or delete the source object. Reparse can reset
	// this same Knowledge row to pending after credits are restored.
	stateCtx := context.Background()
	if err := repo.UpdateKnowledgeColumns(stateCtx, payload.KnowledgeID, map[string]interface{}{
		"parse_status":  types.ParseStatusCreditExhausted,
		"error_message": openRouterCreditExhaustedMessage,
	}); err != nil {
		logger.Warnf(ctx, "openrouter credits: failed to mark knowledge %s credit_exhausted: %v", payload.KnowledgeID, err)
		return
	}
...
// only ingestion stages are allowed to move the document into credit_exhausted.
