...
func (e *AgentEngine) streamThinkingToEventBus(
	...
	logger.Debugf(ctx, "[Agent][Thinking] Iteration-%d: temp=%.2f, tools=%d, thinking=%v, reasoning_effort=%s, max_tokens=%d",
		iteration+1, e.config.Temperature, len(tools), e.config.Thinking, e.config.ReasoningEffort, budget)
