...
func TestPolicyRejectsAlwaysForbiddenTargets(t *testing.T) {
	...
	cases := []struct{ name, raw string }{
		...
		{"benchmarking range upper bound", "http://198.19.255.254"},
...
func TestPolicyAllowsPublicHostname(t *testing.T) {
	...
	// Some hermetic CI/DNS environments rewrite external names into RFC
	// 2544's synthetic benchmarking range. That is deliberately rejected by
	// the production guard, so this public-host smoke test cannot assert a
	// successful lookup there.
	for _, ip := range addrs {
		if isBenchmarkingAddress(ip) {
			t.Skipf("DNS rewrites %s to synthetic benchmarking address %s", host, ip)
		}
	}
...
func TestPolicyDialControlMirrorsValidation(t *testing.T) {
	...
	alwaysBlocked := []string{"169.254.169.254:80", "198.18.0.1:80", "0.0.0.0:80"}
