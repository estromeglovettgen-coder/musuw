...
// isBenchmarkingAddress reports RFC 2544's 198.18.0.0/15 range. It is not
// covered by net.IP.IsPrivate, but operating systems, local DNS shims, and
// test harnesses commonly use it as a synthetic/non-routable address. A
// tenant endpoint resolving there cannot reach a sandbox and must not be
// treated as a public destination. Documentation TEST-NET ranges remain
// intentionally allowed for hermetic configuration/tests.
func isBenchmarkingAddress(ip net.IP) bool {
	v4 := ip.To4()
