...
// SessionBoundedFileReader reads at most maxBytes+1 bytes from one file in a
// session sandbox. The extra byte lets callers distinguish an exact-limit
// file from one that must be rejected, while keeping provider-side materialised
// output bounded. It is an optional capability layered on SessionFileReader so
// older custom managers can continue to compile; callers handling untrusted
// files must require this interface before reading.
type SessionBoundedFileReader interface {
	ReadSessionFileBounded(ctx context.Context, sessionID, path string, maxBytes int64) ([]byte, error)
}
