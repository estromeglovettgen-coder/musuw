...
// ReadSessionFileBounded reads a file through the provider's command/data
// plane with a hard cap of maxBytes+1 bytes. Stat-before-read is deliberately
// not enough here: a skill can grow or replace the file between those calls,
// and the old unbounded ReadFile path would materialise the whole payload in
// the WeKnora process before the caller could reject it. The provider executes
// `head -c` as the sandbox account, so the returned stdout is bounded before it
// crosses the process boundary. The extra byte lets artifact collection reject
// files that exceed its configured max without an unbounded read.
func (m *SessionBoundManager) ReadSessionFileBounded(
	ctx context.Context, sessionID, filePath string, maxBytes int64,
) ([]byte, error) {
	if strings.TrimSpace(filePath) == "" {
		return nil, errors.New("sandbox: path required for ReadSessionFileBounded")
	}
	if maxBytes <= 0 {
		return nil, errors.New("sandbox: maxBytes must be positive for ReadSessionFileBounded")
	}
	if maxBytes == 1<<63-1 {
		return nil, errors.New("sandbox: maxBytes is too large for ReadSessionFileBounded")
	}
	handle, ok, err := m.lookupSessionHandle(ctx, sessionID)
	if err != nil {
		return nil, err
	...
	if !ok {
		return nil, fmt.Errorf("sandbox: no sandbox for session %s", sessionID)
	}
	...
	result, err := m.client.Exec(ctx, handle, RemoteExecRequest{
		...
		Timeout: boundedFileReadTimeout(m.config),
	})
	if err != nil {
		return nil, fmt.Errorf("sandbox: bounded read %s: %w", filePath, err)
	}
	if result == nil {
		return nil, fmt.Errorf("sandbox: bounded read %s returned no result", filePath)
	}
	if result.ExitCode != 0 {
		...
		return nil, fmt.Errorf("sandbox: bounded read %s failed: %s", filePath, stderr)
	}
	// The provider command is expected to enforce this already. Keep a final
	// defensive slice in case a custom adapter ignores argv and returns a few
	// extra bytes; it never turns the read into an unbounded operation.
...
func boundedFileReadTimeout(cfg *Config) time.Duration {
...
var (
	...
	_ SessionBoundedFileReader         = (*SessionBoundManager)(nil)
