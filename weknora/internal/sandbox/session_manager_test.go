...
func TestReadSessionFileBoundedUsesProviderSideCapPlusOne(t *testing.T) {
	ctx := context.WithValue(context.Background(), types.TenantIDContextKey, uint64(10000))
	mgr, client := newSessionManagerExecTestHarness(t)
	_, err := mgr.ExecShellCommand(ctx, "sess-1", "echo warmup", "", time.Second, nil)
	require.NoError(t, err)
	client.mu.Lock()
	client.execRequests = nil
	client.mu.Unlock()

	data, err := mgr.ReadSessionFileBounded(ctx, "sess-1", "/workspace/output/report.txt", 100)
	require.NoError(t, err)
	require.Empty(t, data)

	last := lastExecRequest(t, client)
	require.Equal(t, "head", last.Command)
	require.Equal(t, []string{"-c", "101", "--", "/workspace/output/report.txt"}, last.Args)
	require.Equal(t, DefaultSandboxExecUser, last.User)
	// This request is executed inside the provider sandbox. The command reads
	// only max+1 bytes before stdout crosses into the WeKnora process, so a
	// stale small RemoteDirEntry cannot turn artifact collection into an
