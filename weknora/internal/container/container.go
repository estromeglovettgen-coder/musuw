...
func BuildContainer(container *dig.Container) *dig.Container {
	...
	// Register the deployment-wide sandbox manager. It preserves the legacy
	// environment-driven Docker backend for agents without a named workspace
	// config; named configs are resolved by the tenant resolver below.
	...
	must(container.Provide(service.NewConsumerModelResolver))
	must(container.Provide(service.NewModelServiceWithConsumerResolver))
	...
	must(container.Provide(func(
		...
		return handler.NewSkillHandler(s, s, preloaded)
	}))
