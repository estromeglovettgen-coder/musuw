...
| 名称 | 默认值 | 说明 |
| --- | --- | --- |
| Sandbox 具名配置 | 设置页按空间维护 | provider 身份只来自空间配置；未选择时兼容 `WEKNORA_SANDBOX_MODE=docker/disabled`、`WEKNORA_SANDBOX_TIMEOUT`、`WEKNORA_SANDBOX_DOCKER_IMAGE`，旧 `local` 模式 fail-closed |
...
| --- | --- | --- | --- |
| `rag-qa` | `progressive_rag_agent` | knowledge_search、grep_chunks、list_knowledge_chunks、get_document_info | temperature 0.7、max_iterations 50、FAQ 优先 |
| `wiki-qa` | `wiki_researcher` | wiki_search、wiki_read_page、wiki_read_source_doc、wiki_flag_issue | 需 Wiki 已启用的知识库 |
| `hybrid-rag-wiki` | `hybrid_rag_wiki_agent` | Wiki + RAG 工具全集 | max_iterations 50，最灵活的预设 |
