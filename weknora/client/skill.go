...
// ListSkills lists skills a chat turn can invoke. Pass a sandbox config ID to
// list the skills installed on that named image; omit it to use the deployment
// default (including legacy preloaded skills on env-only Docker deployments).
// The variadic form keeps callers compiled against both the old zero-argument
// and newer one-argument API.
func (c *Client) ListSkills(ctx context.Context, sandboxConfigID ...string) ([]SkillInfo, bool, error) {
	query := url.Values{}
	if len(sandboxConfigID) > 0 && sandboxConfigID[0] != "" {
		query.Set("sandbox_config_id", sandboxConfigID[0])
	}
