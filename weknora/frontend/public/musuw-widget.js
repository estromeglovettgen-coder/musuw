/**
 * Musuw embed widget SDK — floating chat launcher.
 *
 * Programmatic:
 *   Musuw.init({ channel, token, position, primaryColor, title, baseUrl })
 *   Musuw.open() | close() | toggle() | destroy()
 *   Musuw.on('ready', fn) | off('ready', fn)
 *
 * Host context & actions (postMessage to iframe):
 *   Musuw.setContext({ userId, page, ... })
 *     Inject visitor/page context merged into each chat query.
 *   Musuw.openWithQuery('How do I reset my password?')
...
(function (global) {
  ...
  function emit(event, payload) {
    ...
    handlers.slice().forEach(function (fn) {
      try { fn(payload); } catch (e) { console.error('[Musuw]', e); }
    });
  }
  ...
  function createWidget(opts) {
    ...
    if (!channelId || (!staticToken && !tokenEndpoint)) {
      console.warn('[Musuw] channel and (token or tokenEndpoint) are required');
      return null;
    }
    ...
    function loadToken(force) {
      ...
      tokenInFlight = fetch(tokenEndpoint, {
        ...
        .catch(function (e) {
          console.error('[Musuw] failed to load token', e);
          throw e;
        })
    ...
    function setContext(ctx) {
      if (!ctx || typeof ctx !== 'object') {
        console.warn('[Musuw] setContext expects an object');
        return;
      }
    ...
    function openWithQuery(query) {
      ...
      if (!text) {
        console.warn('[Musuw] openWithQuery requires a non-empty query');
        return;
      }
      setOpen(true);
    ...
    function setLocale(locale) {
      ...
      if (!loc) {
        console.warn('[Musuw] setLocale requires a locale string');
        return;
      }
  ...
  global.Musuw = api;
  // Keep existing third-party embeds working while Musuw remains the public
  // name. This alias is a compatibility contract, not a visible brand surface.
  global.WeKnora = api;
