<template>
  <div class="ops-page">
    <header class="ops-page-header">
      <div><h1>日志与追踪</h1><p>复用 WeKnora 原生运行队列与系统审计组件。Langfuse 未配置时明确 unavailable；不会用空表伪装查询成功。</p></div>
    ...
    <div v-if="!config?.providers.langfuse.available" class="ops-callout is-warning" style="margin-bottom:14px"><InfoCircleIcon class="ops-callout__icon"/><div><strong>Langfuse 查询未配置</strong><span>{{ config?.providers.langfuse.reason }}。运行队列、处理 span、request ID 和系统审计仍可从 WeKnora 查看。</span></div></div>
