...
function redirectToExternalAuthStart() {
  if (typeof window === 'undefined') return;
  if (window.location.pathname === EXTERNAL_AUTH_START_PATH) return;
  // Embed 渠道用 Embed token 鉴权，匿名访问不应离开嵌入页
        ...
        try {
          ...
          if (response.success && response.data) {
            ...
            throw {
              ...
              message: response.message || t('error.tokenRefreshFailed'),
            };
          }
        } catch (refreshError: any) {
          // A database recovery, server error, or network interruption says
          // nothing about whether the stored session is invalid. Keep it for
          // an explicit later retry; only the auth endpoint's 401/403 is
          // definitive enough to clear it and re-enter external auth.
          if (!isDefinitiveNativeSessionFailure(refreshError?.status)) {
            processQueue(refreshError, null);
            return Promise.reject(refreshError);
          }

          // 刷新令牌已被服务端明确拒绝，清除所有token并跳转到登录页
