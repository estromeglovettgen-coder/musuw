                ...
                <template v-else-if="detailCache[item.id]?.data">
                  <div class="detail-header">
                    ...
                    <span
                      ...
                      :class="detailCache[item.id].data.type === 'faq' ? 'faq' : 'doc'"
                    >
                      {{
                  ...
                  <div class="detail-meta">
                    <span v-if="detailCache[item.id].data.type === 'faq'">
                      {{
                        $t("mentionDetail.faqCount", {
                          count:
                            detailCache[item.id].data.chunk_count ??
                            detailCache[item.id].data.count ??
                            0,
                        })
                      }}
                    </span>
                    <span v-else>
                      {{
                        $t("mentionDetail.kbCount", {
                          count:
                            detailCache[item.id].data.knowledge_count ??
                            detailCache[item.id].data.count ??
                            0,
                        })
                      }}
                    ...
                    >
                      ...
                      <span
                        ...
                        @click.stop="
                          handleOrgClick(detailCache[item.id].data.org_name || item.orgName)
                        "
                      >
                    ...
                    <span
                      v-if="
                        agentIdForDetail && (detailCache[item.id].data.org_name || item.orgName)
                      "
                      class="detail-readonly-hint"
                    >
                ...
                <template v-else-if="detailCache[item.id]?.data">
                  <div class="detail-header">
                    <span class="detail-name">{{
                      detailCache[item.id].data.title ||
                      detailCache[item.id].data.file_name ||
                      item.name
                    }}</span>
                  </div>
                  <p v-if="detailCache[item.id].data.description" class="detail-desc">
                  ...
                  <div class="detail-meta">
                    ...
                    >
                      ...
                      <span
                        ...
                        @click.stop="
                          handleKbClick(
                            detailCache[item.id].data.knowledge_base_id || (item as any).kbId,
                          )
                        "
                      >
...
const currentGroup = computed(() =>
  mentionGroups.value.find((group) => group.type === currentGroupType.value),
);
const extraGroups = computed(() =>
  mentionGroups.value.filter(
    (group) => group.type !== "kb" && group.type !== "file" && group.count > 0,
  ),
);
...
const fileGroupOffset = computed(
  () => mentionGroups.value.find((group) => group.type === "file")?.offset || 0,
);
...
const updateActiveGroupFromIndex = (index: number) => {
  const group = groupTabs.value.find(
    (item) => index >= item.offset && index < item.offset + item.count,
  );
...
.mention-detail-content .detail-value.clickable:hover {
  ...
  text-decoration-color: var(--td-brand-color, #0b57d0);
}
</style>
