<template>
  <div class="embed-input-box" :class="{ 'is-replying': isReplying }">
    <div v-if="uploadedAttachments.length" class="embed-input-box__files">
      <div
        v-for="(att, index) in uploadedAttachments"
        :key="`${att.file.name}-${index}`"
        class="embed-file-chip"
      >
    ...
    <div class="embed-input-box__bar">
      <div v-if="showWebSearchToggle || showFileUploadToggle" class="embed-input-box__controls">
        ...
        <t-tooltip
          v-if="showFileUploadToggle"
          placement="top"
          :content="t('input.imageUpload.tooltip')"
        >
          <button
            type="button"
        ...
        <t-tooltip
          v-if="showFileUploadToggle"
          placement="top"
          :content="t('input.fileUpload.tooltip')"
        >
          <button
            type="button"
...
.embed-input-box {
  ...
  border: 1px solid var(--musuw-line-strong, #d1d5db);
  box-shadow: 0 1px 2px rgba(17, 24, 39, 0.04), 0 12px 28px rgba(17, 24, 39, 0.08);
  transition: border-color 0.15s ease, box-shadow 0.15s ease;

  &:focus-within {
    border-color: var(--embed-primary, var(--td-brand-color, #0b57d0));
    box-shadow: 0 0 0 3px color-mix(in srgb, var(--embed-primary, #111827) 10%, transparent),
...
.embed-control-btn {
  ...
  &.active {
    color: var(--embed-primary, var(--td-brand-color, #0b57d0));
    background: color-mix(in srgb, var(--embed-primary, #0b57d0) 12%, transparent);
  }
}
...
.embed-send-btn {
  ...
  border-radius: var(--musuw-radius-control, 8px);
  cursor: pointer;
  background: var(--embed-primary, var(--td-brand-color, #0b57d0));
  transition:
    background 0.15s ease,
    opacity 0.15s ease,
