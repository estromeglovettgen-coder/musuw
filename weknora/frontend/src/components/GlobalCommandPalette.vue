  ...
  <t-dialog v-model:visible="dialogVisible" :footer="false" :header="false" :close-btn="false" width="640px" top="10vh"
    ...
    <div class="cmdk" @keydown="onKeyDown">
      <div class="cmdk__input-row">
        ...
        <span v-if="loading" class="cmdk__input-spinner"><t-loading size="small" /></span>
          ...
          <ResultGroup v-if="isGroupVisible('chunks') && fileGroups.length" :label="t('commandPalette.group.chunks')"
            ...
            <template v-for="(item, i) in flatChunkItems" :key="'c-' + item.file.knowledgeId + '-' + item.chunk.id">
              <ResultItem :index="flatIndexFor('chunks', i)" :selected="selectedIndex === flatIndexFor('chunks', i)"
                ...
                <template #subtitle><span v-html="highlight(item.chunk.matched_content || item.chunk.content)" /></template>
              </ResultItem>
            </template>
          </ResultGroup>
          ...
          <ResultGroup v-if="isGroupVisible('messages') && messageGroups.length"
            ...
            <template v-for="(item, i) in flatMessageItems" :key="'m-' + item.msg.request_id">
              <ResultItem :index="flatIndexFor('messages', i)" :selected="selectedIndex === flatIndexFor('messages', i)"
                ...
                <template #title><span>{{ item.group.sessionTitle || t('commandPalette.untitledSession') }}</span></template>
                <template #subtitle>
...
const groupOrder = computed<readonly string[]>(() => {
  if (!query.value.trim()) return ['recent', 'commands'] as const
  if (activeKbScope.value) return ['chunks'] as const
...
interface FlatItem { key: string; group: string; run: (ev?: { cmd: boolean }) => void }
const flatItems = computed<FlatItem[]>(() => {
  const out: FlatItem[] = []
  for (const g of groupOrder.value) {
    if (g === 'recent') recentQueries.value.forEach((q, i) => out.push({ key: `recent:${i}:${q}`, group: g, run: () => { query.value = q } }))
    else if (g === 'commands') {
      const list = query.value.trim() ? filteredCommands.value : allCommands.value
      list.forEach((cmd) => out.push({ key: `cmd:${cmd.id}`, group: g, run: () => cmd.run() }))
    } else if (g === 'chunks') flatChunkItems.value.forEach((item) => out.push({ key: `chunk:${item.chunk.id}`, group: g, run: (ev) => (ev?.cmd ? cmdEnterChunk(item) : openChunk(item)) }))
    else if (g === 'messages') flatMessageItems.value.forEach((item) => out.push({ key: `msg:${item.msg.request_id}`, group: g, run: () => openMessage(item) }))
    else if (g === 'kbs') kbMatches.value.forEach((kb) => out.push({ key: `kb:${kb.id}`, group: g, run: () => openKb(kb.id) }))
    else if (g === 'agents') agentMatches.value.forEach((a) => out.push({ key: `agent:${a.id}`, group: g, run: () => openAgent(a.id) }))
    else if (g === 'sessions') sessionMatches.value.forEach((s) => out.push({ key: `session:${s.id}`, group: g, run: () => openSession(s.id) }))
  }
  return out
})
...
const hasAnyResults = computed(() => groupOrder.value.some((g) => (groupSizes.value[g] || 0) > 0))
const isGroupVisible = (name: string): boolean => groupOrder.value.includes(name as never)
const primaryActionForSelected = (ev?: { cmd: boolean }) => flatItems.value[selectedIndex.value]?.run(ev)

const openChunk = (item: FlatChunkItem) => {
  commandPaletteStore.pushRecent(query.value); commandPaletteStore.closePalette()
  if (!item.file.kbId) return
  const currentKbId = typeof route.params.kbId === 'string' ? route.params.kbId : ''
  if (currentKbId === item.file.kbId) {
    window.dispatchEvent(new CustomEvent('weknora:open-knowledge', { detail: { kbId: item.file.kbId, knowledgeId: item.file.knowledgeId } }))
    return
  }
  router.push({ path: `/platform/knowledge-bases/${item.file.kbId}`, query: { knowledge_id: item.file.knowledgeId } })
}
...
const openMessage = (item: FlatMsgItem) => {
  commandPaletteStore.pushRecent(query.value); commandPaletteStore.closePalette()
  if (item.group.sessionId) router.push(`/platform/chat/${item.group.sessionId}`)
}
const openKb = (kbId: string) => {
  commandPaletteStore.pushRecent(query.value); commandPaletteStore.closePalette(); router.push(`/platform/knowledge-bases/${kbId}`)
}
const openAgent = (agentId: string) => {
  commandPaletteStore.pushRecent(query.value); commandPaletteStore.closePalette(); router.push({ path: '/platform/creatChat', query: { agent_id: agentId } })
}
const openSession = (sessionId: string) => {
  if (!sessionId) return
  commandPaletteStore.pushRecent(query.value); commandPaletteStore.closePalette(); router.push(`/platform/chat/${sessionId}`)
}
const askAi = () => {
  if (!query.value.trim()) return
  commandPaletteStore.pushRecent(query.value); commandPaletteStore.closePalette(); startChat(query.value)
}
const highlight = (text: string) => highlightText(text, query.value)

const scrollSelectedIntoView = () => nextTick(() => scrollRef.value?.querySelector<HTMLElement>(`[data-cmdk-index="${selectedIndex.value}"]`)?.scrollIntoView({ block: 'nearest' }))
...
const selectItemAt = (idx: number) => { const item = flatItems.value[idx]; if (item) selectedKey.value = item.key }
const shortcutFor = (flatIndex: number): number | undefined => flatIndex >= 0 && flatIndex <= 8 ? flatIndex + 1 : undefined
const onKeyDown = (e: KeyboardEvent) => {
  if (e.key === 'ArrowDown') { e.preventDefault(); moveSelection(1) }
  else if (e.key === 'ArrowUp') { e.preventDefault(); moveSelection(-1) }
  else if ((e.metaKey || e.ctrlKey) && e.key >= '1' && e.key <= '9') {
    const item = flatItems.value[parseInt(e.key, 10) - 1]
    if (item) { e.preventDefault(); item.run({ cmd: false }) }
  } else if (e.key === 'Enter') { e.preventDefault(); primaryActionForSelected({ cmd: e.metaKey || e.ctrlKey }) }
  else if (e.key === 'Escape') { e.preventDefault(); handleClose() }
}
const onInputKeyDown = (e: KeyboardEvent) => {
  if (e.key === 'Backspace' && !query.value && activeKbScope.value) { e.preventDefault(); clearKbScope() }
}
const handleClose = () => { drawerVisible.value = false; commandPaletteStore.closePalette() }
...
const onGlobalKey = (e: KeyboardEvent) => {
  ...
  if (isCmd && e.key.toLowerCase() === 'k') {
    e.preventDefault(); if (open.value) handleClose(); else commandPaletteStore.openPalette(''); return
  }
  if (e.key === '/' && !open.value && !isEditingElement(e.target)) { e.preventDefault(); commandPaletteStore.openPalette('') }
}
watch(open, (val) => {
  if (val) {
    query.value = initialQuery.value || ''; selectedKey.value = null; scopeDismissed.value = false
    ...
    if (kbIdFromRoute) {
      ...
      activeKbScope.value = { id: kbIdFromRoute, name: match?.name || kbIdFromRoute }
    } else activeKbScope.value = null
  } else {
    clearResults(); query.value = ''; drawerVisible.value = false; activeKbScope.value = null
  }
})
const focusInputWithRetry = () => {
  ...
  if (tryFocus()) return
  requestAnimationFrame(() => { if (tryFocus()) return; setTimeout(tryFocus, 50) })
}
const onDialogOpened = () => focusInputWithRetry()
...
const clearKbScope = () => { activeKbScope.value = null; scopeDismissed.value = true; nextTick(() => inputRef.value?.focus()) }
onMounted(() => window.addEventListener('keydown', onGlobalKey))
onUnmounted(() => window.removeEventListener('keydown', onGlobalKey))
</script>

<style lang="less" scoped>
.cmdk { display: flex; flex-direction: column; gap: 0; min-height: 320px; max-height: 60vh; }
.cmdk__input-row { display: flex; align-items: center; gap: 8px; padding: 12px 14px; border-bottom: 1px solid var(--td-component-stroke); }
.cmdk__input-icon { color: var(--td-text-color-placeholder); font-size: 16px; }
.cmdk__scope-chip { display: inline-flex; align-items: center; gap: 6px; max-width: 220px; height: 26px; padding: 0 2px 0 8px; background: var(--td-bg-color-secondarycontainer); color: var(--td-text-color-primary); border-radius: 4px; font-size: 12px; font-weight: 500; flex-shrink: 0; }
.cmdk__scope-chip :deep(.t-icon:first-child) { color: var(--td-text-color-secondary); }
.cmdk__scope-chip-name { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.cmdk__scope-chip-x { display: inline-flex; align-items: center; justify-content: center; width: 20px; height: 20px; border: none; border-radius: 3px; background: transparent; color: var(--td-text-color-placeholder); cursor: pointer; line-height: 1; padding: 0; }
.cmdk__scope-chip-x:hover { color: var(--td-text-color-primary); background: rgba(0,0,0,.05); }
.cmdk__input { flex: 1; border: none; outline: none; background: transparent; font-size: 15px; color: var(--td-text-color-primary); font-family: inherit; }
.cmdk__input::placeholder { color: var(--td-text-color-placeholder); }
.cmdk__input-spinner { display: flex; align-items: center; }
.cmdk__icon-btn { display: inline-flex; align-items: center; justify-content: center; width: 28px; height: 28px; border: none; border-radius: 6px; background: transparent; color: var(--td-text-color-secondary); cursor: pointer; transition: background .1s; }
.cmdk__icon-btn:hover,.cmdk__icon-btn.active { background: var(--td-bg-color-secondarycontainer); color: var(--td-text-color-primary); }
.cmdk__results { flex: 1; min-height: 0; overflow-y: auto; padding: 6px 4px; }
.cmdk__empty { display: flex; flex-direction: column; align-items: center; gap: 12px; padding: 40px 20px 20px; color: var(--td-text-color-placeholder); font-size: 13px; }
.cmdk__empty p { margin: 0; }
.cmdk__empty-actions { display: flex; gap: 8px; }
.cmdk__footer { display: flex; gap: 16px; padding: 8px 14px; border-top: 1px solid var(--td-component-stroke); font-size: 11px; color: var(--td-text-color-placeholder); flex-wrap: wrap; }
.cmdk__hotkey { display: inline-flex; align-items: center; gap: 4px; }
.cmdk__hotkey kbd { display: inline-block; padding: 1px 5px; min-width: 16px; font-size: 10px; font-family: inherit; line-height: 14px; text-align: center; background: var(--td-bg-color-secondarycontainer); border: 1px solid var(--td-component-stroke); border-radius: 3px; color: var(--td-text-color-secondary); }
.cmdk-chunk-kb { font-size: 11px; color: var(--td-text-color-placeholder); padding: 1px 6px; background: var(--td-bg-color-secondarycontainer); border-radius: 3px; font-weight: 400; }
.cmdk-chunk-title { overflow: hidden; text-overflow: ellipsis; }
.cmdk-msg-role { display: inline-block; margin-right: 6px; padding: 0 5px; font-size: 10px; font-weight: 600; background: var(--td-bg-color-secondarycontainer); color: var(--td-text-color-secondary); border-radius: 3px; }
</style>

<style lang="less">
.cmdk-dialog .t-dialog__body { padding: 0; }
.cmdk-dialog .t-dialog { padding: 0; overflow: hidden; }
.cmdk-retrieval-drawer .section-header { font-weight: 600; }
</style>
