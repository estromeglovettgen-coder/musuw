export default {
  ...
  knowledgeBase: {
    ...
    urlTip: 'Вставьте URL или текст публикации с поддерживаемой платформы',
    urlSupportedPlatforms: 'Поддерживаются',
    urlInputHint: 'Ссылка определяется автоматически: видео импортируется как файл, публикация — как документ. Поддерживаются только отдельные публичные публикации.',
    urlClear: 'Очистить',
    urlTooLong: 'URL или текст публикации не должен превышать 4 КБ',
    douyinTikTok: 'Douyin · TikTok',
    urlUsageNotice: 'Импортируйте только контент, которым вы владеете или на использование которого у вас есть разрешение. Импорт URL предназначен для личного индекса знаний, а не для стриминга, скачивания или распространения контента.',
    typeURL: 'URL',
    typeManual: 'Вручную',
    typeFile: 'Файл',
  ...
  contextualGuide: {
    ...
    kbDetail: {
      steps: {
        ...
        uploadUrl: {
          title: 'Импортировать веб-страницу',
          desc: 'Используйте импорт для открытой веб-страницы или ссылки из соцсети; действующие правила тарифа будут показаны перед использованием.'
        },
        intro: {
