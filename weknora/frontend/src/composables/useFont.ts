...
function applyFont() {
  ...
  if (!root) return
  // The archived Musuw typefaces are declared by theme.css. Only write an
  // inline value after a user explicitly chooses a font; otherwise the
  // application default remains the stylesheet source of truth.
  const savedSans = loadPreference(SANS_KEY)
  const savedMono = loadPreference(MONO_KEY)
  const sansStack = isFontKey(savedSans)
    ? (SANS_STACKS[currentSans.value] ?? SANS_STACKS[DEFAULT_SANS])
    : ''
  const monoStack = isMonoFontKey(savedMono)
    ? (MONO_STACKS[currentMono.value] ?? MONO_STACKS[DEFAULT_MONO])
    : ''
  const scale = String(FONT_SCALES[currentSize.value] ?? FONT_SCALES[DEFAULT_SIZE])
  if (lastApplied.sans !== sansStack) {
    if (sansStack) root.style.setProperty('--app-font-family', sansStack)
    else root.style.removeProperty('--app-font-family')
    lastApplied.sans = sansStack
  }
  if (lastApplied.mono !== monoStack) {
    if (monoStack) root.style.setProperty('--app-font-family-mono', monoStack)
    else root.style.removeProperty('--app-font-family-mono')
