...
const executeUploadBatch = async (
  ...
  options: {
    tagIds?: string[];
    /** Destination folder selected in the knowledge-base tree; '' is the root. */
    targetFolder?: string;
  } = {},
) => {
...
const executeUrlImport = async (
  ...
  try {
    const uploadData: { url: string; tag_ids?: string[] } = {
      url,
...
const startPlatformDefaultUpload = async (files: File[], urls: string[] = []) => {
  ...
  for (const url of urls) {
    await executeUrlImport(url, tagIds);
  }
};
...
<template>
  <template v-if="!isFAQ">
    <div
      ...
      :class="{ 'documents-layout': activeKbTab === 'documents' || !isWiki }"
    >
...
.knowledge-layout.documents-layout {
  gap: 16px;
  padding: 20px 28px 0;
}
...
.documents-layout .document-header {
  padding-bottom: 14px;
  border-bottom: 1px solid var(--td-component-stroke);
}
