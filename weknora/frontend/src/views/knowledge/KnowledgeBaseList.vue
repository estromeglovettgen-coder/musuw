    ...
    <div class="kb-list-content">
      <div class="header" style="--wails-draggable: drag">
        <div class="header-title" style="--wails-draggable: drag">
          ...
          <p class="header-subtitle" style="--wails-draggable: drag">
        ...
        <div v-if="uploadSummaries.length" class="upload-progress-panel">
          <div v-for="summary in uploadSummaries" :key="summary.kbId" class="upload-progress-item">
            <div class="upload-progress-icon">
              <t-icon
                :name="summary.completed === summary.total ? 'check-circle-filled' : 'upload'"
                size="20px"
              />
            </div>
        ...
        >
          ...
          <div
            v-if="
              ...
              filteredKnowledgeBases[0].isMine &&
              filteredKnowledgeBases[0].is_pinned
            "
            ...
            >
              ...
              <div class="card-header">
                ...
                <!-- The card menu always exists when the card is visible: pin
                     ...
                     (backend route only requires KB read access). Delete stays
                     behind canManageKBCard. -->
                <t-popup
                  ...
                  <template #content>
                    <div class="popup-menu" @click.stop>
                      <div class="popup-menu-item" @click.stop="handleTogglePinById(kb.id)">
                        ...
                        <span>{{
                          kb.is_pinned ? $t("knowledgeList.pin.unpin") : $t("knowledgeList.pin.pin")
                        }}</span>
                      </div>
                      <div
          ...
          <button
            ...
            type="button"
            class="kb-card kb-create-card"
            data-guide="kb-list-create"
            @click="handleCreateKnowledgeBase"
          >
            <span class="kb-create-card-icon" aria-hidden="true">
              <t-icon name="folder-add" size="18px" />
            </span>
            <span class="kb-create-card-label">{{ $t("knowledgeList.create") }}</span>
          </button>
        </div>
        ...
        <div v-else-if="spaceSelectionOrgId && sortedSpaceKbsList.length > 0" class="kb-card-wrap">
          <template
            ...
            :key="
              'shared-' +
              (shared.share_id ||
                `agent-${shared.knowledge_base?.id}-${shared.source_from_agent?.agent_id || ''}`)
            "
          >
            ...
            >
              ...
              <div class="card-header">
                ...
                <t-tooltip
                  v-if="!shared.is_mine"
                  :content="$t('knowledgeList.menu.viewDetails')"
                  placement="top"
                >
                  <button
                    type="button"
                  ...
                  <div class="feature-badges">
                    ...
                    >
                      <div
                        ...
                        :class="{
                          'type-document':
                            (shared.knowledge_base.type || 'document') === 'document',
                          'type-faq': shared.knowledge_base.type === 'faq',
                        }"
                      >
                        <t-icon
                          :name="
                            shared.knowledge_base.type === 'faq' ? 'chat-bubble-help' : 'folder'
                          "
                          size="14px"
                        />
                        <span class="badge-count">{{
            ...
            <div class="shared-detail-drawer-body">
              ...
              <div v-if="currentSharedKbForDetail.source_from_agent" class="shared-detail-row">
                ...
                <span class="shared-detail-value">
                  {{
                    agentKbStrategyText(
                      currentSharedKbForDetail.source_from_agent?.kb_selection_mode ?? "",
                    )
                  }}
                </span>
              </div>
...
// 左侧空间选择：默认根据当前角色决定。
// The managed product has one knowledge-base view: the current workspace.
// Keep the existing URL-state composable, but collapse every legacy scope
// and creator deep link onto that canonical view.
const defaultScope: "mine" = "mine";
...
interface KB {
  ...
  // gating the per-card more-menu actions. Empty for legacy
...
type KbSectionKey =
  "pinned" | "mine" | "tenantOthers" | "sharedByMe" | "sharedEditable" | "sharedReadonly";
...
// destructive Delete item of the per-card menu so a
  ...
  try {
    ...
    if (res.success) {
      MessagePlugin.success(
        res.data.is_pinned
          ? t("knowledgeList.pin.pinSuccess")
          : t("knowledgeList.pin.unpinSuccess"),
      );
      fetchList(true);
    }
  } catch {
...
type SharedKbDetailItem = SharedKnowledgeBase & {
  is_mine?: boolean;
...
const isWikiKb = (kb: unknown) =>
  !!(kb as { indexing_strategy?: { wiki_enabled?: boolean } } | null | undefined)?.indexing_strategy
    ?.wiki_enabled;
...
const uploadSummaries = computed<UploadSummary[]>(() => {
  ...
  return Object.entries(grouped)
    .map(([kbId, tasks]) => {
      ...
      const avgProgress =
        total === 0 ? 0 : Math.min(100, Math.max(0, Math.round(progressSum / total)));
...
.kb-create-card {
  ...
  &:focus-visible {
    outline: 2px solid color-mix(in srgb, var(--td-brand-color) 50%, transparent);
    outline-offset: 2px;
  }
