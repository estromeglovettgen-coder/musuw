    ...
    <div ref="scrollContainer" class="visual-chat-scroll" @scroll="handleScroll">
      <div class="visual-chat-messages" :class="{ 'is-embedded': embeddedMode }">
        <div v-if="historyLoading && messagesList.length === 0" class="visual-chat-skeletons" aria-hidden="true">
          <div class="visual-chat-skeleton is-user">
            <t-skeleton animation="gradient" :row-col="[{ width: '45%', height: '36px', type: 'rect' }]" />
          </div>
          <div class="visual-chat-skeleton is-assistant">
            <t-skeleton
              animation="gradient"
              :row-col="[
        ...
        <section
          v-if="!embeddedMode && messagesList.length === 0 && !loading"
          class="visual-chat-suggestions"
        ...
        >
          <div v-if="suggestedQuestionsLoading && suggestedQuestions.length === 0" class="visual-chat-suggestions__inner">
            <div class="visual-chat-suggestions__caption">
              <t-skeleton animation="gradient" :row-col="[{ width: '120px', height: '14px' }]" />
            </div>
            <div class="visual-chat-suggestions__grid" aria-hidden="true">
              <div v-for="n in 6" :key="`sq-skel-${n}`" class="visual-chat-suggestion is-skeleton">
          ...
          <Transition v-else appear name="visual-chat-suggestions-fade">
            <div v-if="suggestedQuestions.length > 0" class="visual-chat-suggestions__inner">
              <p class="visual-chat-suggestions__caption">
                <span>{{ t('chat.suggestedQuestions') }}</span>
                <button
                  type="button"
                  class="visual-chat-suggestions__refresh"
                ...
                >
                  <t-icon
                    :name="suggestedQuestionsLoading ? 'loading' : 'refresh'"
                    :class="{ 'is-spinning': suggestedQuestionsLoading }"
                  />
                </button>
              </p>
              <div class="visual-chat-suggestions__grid">
                <button
                  v-for="item in suggestedQuestions"
                  :key="item.question"
                  type="button"
                  class="visual-chat-suggestion"
                  @click="handleSuggestedQuestionClick(item.question)"
                >
                  <span>{{ item.question }}</span>
                  <small v-if="item.source === 'faq'">FAQ</small>
                </button>
              </div>
            </div>
          </Transition>
        ...
        <div v-if="showGlobalTypingIndicator" class="visual-chat-wait" role="status" :aria-label="t('chat.thinkingAlt')">
          <span aria-hidden="true" />
        </div>
      </div>
    </div>

    <Transition name="visual-chat-scroll-button">
      <button
        v-show="userHasScrolledUp"
        type="button"
        class="visual-chat-scroll-bottom"
...
.visual-chat-view {
  ...
  background: #fff;
  color: #1f2937;
  transition: padding-right 220ms cubic-bezier(.22,.61,.36,1);
}

.visual-chat-view.has-references-panel:not(.is-embedded) {
  @media (min-width: 960px) {
    padding-right: 420px;
  }
}

.visual-chat-view.is-embedded {
  min-width: 100%;
  max-width: 100%;
}
...
.visual-chat-view.has-references-panel .visual-chat-scroll {
  @media (min-width: 960px) {
    padding-top: 18px;
  }
}

.visual-chat-view.is-embedded .visual-chat-scroll {
  padding: 16px;
}
...
.visual-chat-message-row { min-width: 0; contain: layout style; }

.visual-chat-skeletons { display: flex; flex-direction: column; gap: 20px; padding: 12px 0; }
...
.visual-chat-suggestions__inner { display: flex; flex-direction: column; gap: 10px; }
.visual-chat-suggestions__caption { min-height: 20px; margin: 0; display: flex; align-items: center; justify-content: center; gap: 5px; color: #9ca3af; font-size: 10px; line-height: 16px; }
.visual-chat-suggestions__refresh { width: 20px; height: 20px; padding: 4px; border: 0; border-radius: 6px; display: inline-flex; align-items: center; justify-content: center; background: transparent; color: #9ca3af; cursor: pointer; }
.visual-chat-suggestions__refresh:hover:not(:disabled) { background: #f3f4f6; color: #4b5563; }
.visual-chat-suggestions__refresh :deep(.t-icon) { font-size: 11px; }
.visual-chat-suggestions__refresh .is-spinning { animation: visual-chat-spin .8s linear infinite; }
.visual-chat-suggestions__grid { display: flex; flex-wrap: wrap; justify-content: center; gap: 8px; }
.visual-chat-suggestion { min-height: 34px; max-width: 100%; padding: 7px 11px; border: 1px solid #e5e7eb; border-radius: 10px; display: inline-flex; align-items: center; gap: 6px; background: #fff; color: #4b5563; font: inherit; font-size: 10px; line-height: 16px; cursor: pointer; }
.visual-chat-suggestion:hover:not(.is-skeleton) { border-color: #d1d5db; background: #f9fafb; color: #111827; }
.visual-chat-suggestion small { padding: 1px 5px; border-radius: 5px; background: #f3f4f6; color: #9ca3af; font-size: 8px; }
...
.visual-chat-scroll-button-enter-active,
.visual-chat-scroll-button-leave-active { transition: opacity 140ms ease, transform 140ms ease; }
.visual-chat-scroll-button-enter-from,
.visual-chat-scroll-button-leave-to { opacity: 0; transform: translateX(-50%) translateY(5px); }
.visual-chat-suggestions-fade-enter-active,
.visual-chat-suggestions-fade-leave-active { transition: opacity 140ms ease; }
.visual-chat-suggestions-fade-enter-from,
.visual-chat-suggestions-fade-leave-to { opacity: 0; }

@keyframes visual-chat-spin { to { transform: rotate(360deg); } }

@media (max-width: 720px) {
  .visual-chat-scroll { padding-inline: 16px; }
  .visual-chat-input { width: calc(100% - 32px); }
}

@media (prefers-reduced-motion: reduce) {
  .visual-chat-view,
  .visual-chat-scroll-button-enter-active,
  .visual-chat-scroll-button-leave-active,
  .visual-chat-suggestions-fade-enter-active,
  .visual-chat-suggestions-fade-leave-active { transition: none !important; }
  .visual-chat-wait > span,
  .visual-chat-suggestions__refresh .is-spinning { animation: none; }
}
</style>
