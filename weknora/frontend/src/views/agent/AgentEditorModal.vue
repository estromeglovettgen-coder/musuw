...
// Musuw Lite intentionally keeps the compact Agent editor surface. Keep this
// allow-list separate from the Standard set so an initial section/deep-link
// can never reveal a Standard-only panel (for example retrieval when a KB is
// present).
...
function resolveEditorSection(section?: string | null): string {
  ...
  return visibleSections.has(key) ? key : 'basic';
}
...
const navItems = computed(() => {
  ...
  // Musuw Lite keeps the compact editor contract. MCP remains available for
  // smart-reasoning agents, while retrieval/web-search/suggestions and other
  // Standard-only tabs stay out of the reachable navigation entirely.
...
watch(currentSection, (section) => {
  // Navigation is the product-level capability boundary. Standard sections
  // stay mounted for reversibility, but Lite must not reach them through an
  // old deep-link or a validation/error redirect.
  const resolved = resolveEditorSection(section);
