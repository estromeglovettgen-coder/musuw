...
test('new agents inherit every platform default model without enabling optional media', () => {
  const defaultBlock = source.match(
    /const applyDefaultModelsIfEmpty = \(\) => \{([\s\S]*?)^\}/m
  )?.[1]

  assert.ok(defaultBlock, 'expected one native default-model initializer')
  for (const type of ['KnowledgeQA', 'Rerank', 'VLLM', 'ASR']) {
    assert.match(defaultBlock, new RegExp(`pick\\('${type}'\\)`))
  }
  for (const field of ['model_id', 'rerank_model_id', 'vlm_model_id', 'asr_model_id']) {
    assert.match(defaultBlock, new RegExp(`config\\.${field}`))
  }
  assert.doesNotMatch(defaultBlock, /image_upload_enabled\s*=\s*true/)
  assert.doesNotMatch(defaultBlock, /audio_upload_enabled\s*=\s*true/)
})
