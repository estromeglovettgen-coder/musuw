...
test('rebuilt answer reference summary preserves grouping, web links, drawer handoff and KB navigation', () => {
  ...
  for (const token of ["referencesDrawer.open({ references: refs })","item.chunk_type === 'web_search'","item.chunk_type !== 'web_search'",'const key = item.knowledge_id || item.knowledge_title || item.id','title: item.knowledge_title || item.knowledge_filename || key','knowledgeId: item.knowledge_id','knowledgeBaseId: item.knowledge_base_id','const sanitized = sanitizeHTML(content)','if (group.knowledgeId) query.knowledge_id = group.knowledgeId','path: `/platform/knowledge-bases/${group.knowledgeBaseId}`','return router.resolve({',':href="getDocumentHref(group)"','target="_blank"','rel="noopener noreferrer"','class="visual-answer-references"']) {
    assert.ok(source.includes(token), `docInfo lost reference contract: ${token}`)
  }
  for (const legacy of ['class="refer"', 'class="refer_header"', 'class="doc-group"', 'class="doc doc-web"']) assert.equal(source.includes(legacy), false)
})
...
test('rebuilt citation hover card preserves web and document state semantics', () => {
  ...
  for (const token of ["float.type === 'web'",':href="float.url"','target="_blank"','rel="noopener noreferrer"','float.loading','float.error','float.content','@mouseenter="onEnter?.()"','@mouseleave="onLeave?.()"','class="visual-citation-float"']) assert.ok(source.includes(token), `ChatCitationFloat lost ${token}`)
...
test('mechanical DOM contracts remain only for still-unmigrated roots', () => {
  ...
  const rebuiltBot = read('../views/chat/components/botmsg.vue')
  assert.equal(rebuiltBot.includes('class="bot_msg"'), false)
  assert.match(rebuiltBot, /class="visual-assistant-message"/)
})
