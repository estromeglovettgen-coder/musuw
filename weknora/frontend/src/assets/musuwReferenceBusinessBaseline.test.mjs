...
const contracts = new Map([
  ...
  ['../components/ModelDebugDrawer.vue', ['props.models.find(model => model.id === selectedModelId.value)','props.models.filter(model => model.type === selectedModelType.value)','modelSupportsThinking(selectedModel.value)','fileSizeVerification(selectedFile)','await debugModel(selectedModel.value.id, {','history.value.unshift({ id: ++runSequence','history.value = history.value.slice(0, 6)','await navigator.clipboard.writeText(JSON.stringify(result.value, null, 2))']],
  ['../views/chat/components/usermsg.vue', ['hydrateProtectedFileImages','isPreviewableAttachment','resolveAttachmentFileType','attachmentPreviewDrawer.open({','sessionId: props.sessionId','attachmentId: attachment.id','reviewImg.value = true','reviewUrl.value = src']],
  ['../views/chat/components/docInfo.vue', ["referencesDrawer.open({ references: refs })","item.chunk_type === 'web_search'","item.chunk_type !== 'web_search'",'const key = item.knowledge_id || item.knowledge_title || item.id','knowledgeId: item.knowledge_id','knowledgeBaseId: item.knowledge_base_id','const sanitized = sanitizeHTML(content)','if (group.knowledgeId) query.knowledge_id = group.knowledgeId','path: `/platform/knowledge-bases/${group.knowledgeBaseId}`']],
  ['../views/chat/components/botmsg.vue', ['useChatCitationPopover(parentMd','useTypewriter(','renderChatMarkdown(text','sanitizeHtml: sanitizeMarkdownHTML','knowledgeReferences: props.session?.knowledge_references','Boolean(props.session?.is_completed) && typedAnswer.value.length >= answerText.value.length','await copyTextToClipboard(content)','buildManualMarkdown(question, content)','uiStore.openManualEditor({','hydrateProtectedFileImages(parentMd.value)','enhanceMarkdownContainer(parentMd.value)','renderMermaidInContainer(parentMd.value)','rebindCitations()']],
