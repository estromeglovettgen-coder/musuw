...
.scroll-to-bottom-btn {
    ...
    justify-content: center;
    cursor: pointer;
    color: var(--musuw-accent, var(--td-text-color-secondary));
    transition: all 0.16s ease;

    &:hover {
        background: var(--td-bg-color-container-hover);
        color: var(--td-text-color-primary);
        box-shadow: var(--musuw-shadow-raised, 0 4px 12px rgba(38, 38, 38, 0.08));
    }
...
.msg-skeleton-list {
    ...
    max-width: 960px;
    padding: 16px 0;
    animation: contentFadeIn 0.16s ease-out;
}
...
.suggested-questions-container {
    transition: min-height 0.16s @suggested-ease;
}
...
.sq-fade-leave-active {
    transition: opacity 0.16s @suggested-ease;
}
