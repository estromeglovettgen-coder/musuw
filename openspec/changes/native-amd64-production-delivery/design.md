...
- [The same-run GitHub source artifact is missing, expired, malformed, or its official blob path is unavailable] → Identity, size, redirect, ZIP, and dual-digest checks fail before dependency work, image publication, or Tokyo mutation; do not fall back to a mutable branch, codeload, unofficial mirror, or custom proxy.
