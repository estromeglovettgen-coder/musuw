...
### Delegate initial transaction creation to Paddle Checkout

For Free-to-paid self-service checkout, the authenticated server returns one allow-listed Live price plus tenant/price signed custom data. Paddle.js creates the transaction from `items` and `customData`, and Paddle Checkout owns incomplete and abandoned transaction lifecycle. Musuw stores no initial checkout operation and grants no entitlement from the browser. Only the signed active subscription notification, whose custom data Paddle copies from the transaction, can initialize paid access.

Server-created initial transactions were rejected because Paddle.js already provides the standard self-service transaction path and the duplicate local operation state added no authority. The narrow serialized operation remains only for the paid subscription-update API until the account has an enabled official Customer Portal Product Collections capability that can fully replace the custom upgrade entry point.
