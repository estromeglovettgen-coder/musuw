...
- Recover an environment-local orphaned paid identity only when the selected Paddle SDK returns typed `not_found` and every local cadence/paid/credit period is absent, by offering the authenticated tenant same-or-higher bound checkout options while leaving signed active subscription events as the sole initial recovery authority.
