...
Musuw already has tenant storage quotas, tenant-scoped repositories, native model factories, and one OpenRouter-backed provider path. It lacks a consumer plan source of truth, per-tenant model/spend enforcement, and a real relationship between the storefront's four price cards and product behavior. Paddle Live has not been authorized for launch; the current checked deployment stage therefore uses Sandbox as one complete environment unit, while payment setup remains optional outside Musuw's fixed production overlay and must not weaken enforcement.
...
The same official SDK fills the one gap in Paddle's hosted portal: changing a paid tier. The client sends only a higher target plan. The server resolves the hidden customer and subscription from the authenticated tenant, fetches the current Paddle subscription to prove ownership and current server-owned price, preserves its monthly/yearly period, and replaces the single item with the mapped target price. Paddle previews and applies the change using `prorated_immediately` and `prevent_change`; the UI shows Paddle's localized minor-unit result before confirmation. The update also rotates the tenant/target-price HMAC in subscription custom data so the existing signed `subscription.updated` webhook remains the sole plan authority. No direct UI callback or update response grants Pro/Max, and no downgrade, term switch, parallel subscription for a provider-resolvable tenant, ledger, or reconciliation process is added.

Generic WeKnora deployments may leave Paddle unconfigured. The shared shape validator understands Sandbox `test_` + `pdl_sdbx_apikey_` and Live `live_` + `pdl_live_apikey_`, each with one destination-specific notification secret and six distinct price IDs. Musuw's fixed production Compose wrapper is intentionally stricter for the current launch: both the preflight and the app entrypoint require Sandbox and reject a complete, internally consistent Live unit because Live is not authorized. Server secrets never enter the generated environment or browser bundle. Because `pri_` IDs and `pdl_ntfset_` secrets do not encode environment, a provider-side Sandbox read of all six recurring prices plus a signed event from the exact Sandbox destination is still required before billing is declared operational. A later Live cutover requires a reviewed code change to the fixed launch wrapper, not an environment-only edit.

Paddle customer and subscription IDs are also environment-local. Switching the SDK from Live to Sandbox cannot migrate or reinterpret a stored identity. The Tokyo case has a paid active tenant whose stored subscription returns Paddle's typed `not_found` in both environments and whose cadence, paid-term end, and credit-period end are all absent. This is a provider-proven orphan reference, not a normal active subscription. Only for that narrow state, the authenticated entitlement read performs one five-second-bounded provider proof and may expose tenant-bound hosted-checkout options for the same or a higher server-owned plan. A timeout, authentication/authorization failure, 5xx, nil response, ownership mismatch, existing cadence/period, inactive status, or a subscription that still resolves never enables recovery. The same proof gates Portal availability for an unconfirmed paid identity: only a resolved matching subscription permits a direct Portal session; orphaned or unverifiable state fails before provider session creation. Normal paid tenants still receive no second checkout. The recovery response writes nothing. A newer correctly signed target-environment active subscription event with the tenant/price binding remains the sole authority that may replace the stale identity and restore the initial plan, cadence, and confirmed period; matching signed recurring transaction events remain the sole authority for later allowance refreshes.
...
- [Stored Paddle IDs are orphaned across environments] → Require an active paid tenant with every billing-period field absent and Paddle's selected-environment typed `not_found`; then expose only same-or-higher tenant-bound recovery checkout. Every other provider outcome fails closed, and signed subscription/transaction events remain the only state authority.
...
4. Configure the Sandbox Paddle deployment unit: matching least-privilege Sandbox server API key and `test_` public client token, the exact Sandbox destination secret, and all six Sandbox recurring price IDs. Resolve all six prices through Sandbox, register `/api/v1/billing/paddle/webhook` for the subscription lifecycle events and `transaction.completed`, and prove the destination with a signed delivery before calling billing operational. For a paid tenant whose stored subscription is a typed `not_found` and whose cadence/paid/credit periods are all absent, use the provider-proven recovery checkout for the same or a higher plan; do not edit the database. Switch to Live only after authorization through a reviewed code change that replaces the entire unit together.
