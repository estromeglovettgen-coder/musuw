...
- Prevent duplicate checkout and upgrade mutations with server-created/reused transaction intent and serialized/reused operation state, and never blindly repeat a provider mutation after an uncertain response.
...
`past_due` remains paid grace through the last confirmed paid-term boundary. It does not create a new allowance or move the boundary until a matching successful recurring payment is verified. Refund/chargeback entitlement policy, adjustment-event handling, Live cutover, and Retain feature activation are explicitly deferred to the same later reviewed Live-readiness change; the current production contract remains the complete Paddle Sandbox unit, and the existing non-authoritative `pwCustomer` handoff does not enable Retain features.
...
- [A checkout or upgrade request times out after the provider may have accepted it] → Mark the operation uncertain, keep the tenant mutation slot occupied, and require explicit provider/operator reconciliation; never issue a blind second mutation or build an automatic reconciler for the Sandbox-only stage.
...
5. Configure the Sandbox Paddle deployment unit: matching least-privilege Sandbox server API key and `test_` public client token, the exact Sandbox destination secret, and all six Sandbox recurring price IDs. Resolve all six prices through Sandbox, register `/api/v1/billing/paddle/webhook` for the subscription lifecycle events and successful recurring completion, and prove the destination with a signed delivery before calling billing operational. Validate the launch with a fresh disposable Sandbox tenant. Refund/chargeback policy, Live cutover, and Retain feature activation remain deferred and require a reviewed change.
