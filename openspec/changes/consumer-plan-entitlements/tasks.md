...
- [x] 4.14 Render Paddle's localized formatted price without rewriting it, pass only the authenticated tenant-derived Paddle customer to `pwCustomer`/`Paddle.Update`, and record the no-charge Live-readiness audit while preserving the fixed-production Sandbox-only lock
...
- [x] 4.17 Implement server-created/reused checkout transaction intents and serialized/reused tenant upgrade operations; keep uncertain Paddle responses fail-closed for explicit reconciliation, never retry them blindly, and accept exactly one known subscription base item
- [ ] 4.18 Verify repeated checkout/upgrade requests across replicas, uncertain provider responses, operation reuse, unsafe multi-item/unknown-item subscriptions, and signed-webhook authority in the Paddle Sandbox
- [ ] 4.19 Before Live authorization, decide and implement the refund/chargeback entitlement policy with Paddle adjustment events and official provider-state reads; do not add this reconciliation subsystem to the Sandbox-only release
- [ ] 4.20 Before Live authorization, verify refund, chargeback, reversal, and Retain behavior in the provider environment selected for that separately reviewed cutover; keep the current production unit Sandbox-only
