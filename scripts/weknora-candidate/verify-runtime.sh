#!/usr/bin/env bash
# Runtime acceptance for the isolated v0.7.2 candidate. It prints only safe
...
app_image="weknora-v072-candidate-app:3d5d8bf"
...
if ! docker image inspect "$app_image" >/dev/null; then
    printf '%s\n' 'candidate v0.7.2 app image is unavailable' >&2
    exit 1
fi
...
if [ "$image_version" != 'v0.7.2' ] || [ "$image_revision" != '3d5d8bfcdfeeea266b292b71cea616847af28d0f' ]; then
    printf '%s\n' 'candidate image provenance labels are not v0.7.2 official source' >&2
    exit 1
fi
...
docker run --rm --entrypoint /bin/sh "$app_image" -ec '
    ...
    grep -aq "3d5d8bfcdfeeea266b292b71cea616847af28d0f" /app/WeKnora
...
printf '%s\n' "candidate runtime green: v0.7.2 binary/provenance, native health/UI/OIDC PKCE, migration ${latest_migration_version}, no legacy service"
