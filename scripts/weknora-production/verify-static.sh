...
for name in db_password redis_password system_aes_key jwt_secret neo4j_auth oidc_client_id oidc_client_secret searxng_secret deepseek_api_key openrouter_api_key openrouter_management_api_key r2_access_key_id r2_secret_access_key; do
...
if grep -Eq '^(DB_PASSWORD|REDIS_PASSWORD|SYSTEM_AES_KEY|JWT_SECRET|NEO4J_AUTH|OIDC_AUTH_CLIENT_SECRET|SEARXNG_SECRET|OPENROUTER_MANAGEMENT_API_KEY)=' "$runtime_dir/production.env"; then
...
for secret in deepseek_api_key openrouter_api_key openrouter_management_api_key r2_access_key_id r2_secret_access_key; do
