...
fail_contract "production authorization must create one deterministic exact-SHA tarball without executing source" unless production_source_bundle&.dig("env", "RELEASE_SHA") == "${{ steps.resolve.outputs.release_sha }}" && source_bundle_run.include?('archive="$bundle_dir/musuw-source.tar.gz"') && source_bundle_run.include?('git archive --format=tar --prefix=musuw-source/ "$RELEASE_SHA"') && source_bundle_run.include?("gzip -n") && source_bundle_run.include?("shasum -a 256") && source_bundle_run.include?("archive_digest=") && !source_bundle_run.include?("provenance.json")
...
%w[--connect-timeout --max-time].each do |option|
  fail_contract "production source artifact is missing bounded curl option #{option}" unless source_archive_run.include?(option)
end
